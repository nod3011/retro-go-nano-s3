...
#define MAP_HIROM_SRAM_OR_NONE                                                 \
  (Memory.SRAMSize == 0 ? (uint8_t *)MAP_NONE : (uint8_t *)MAP_HIROM_SRAM)
#define MAP_LOROM_SRAM_OR_NONE                                                 \
  (Memory.SRAMSize == 0 ? (uint8_t *)MAP_NONE : (uint8_t *)MAP_LOROM_SRAM)
#define MAP_RONLY_SRAM_OR_NONE                                                 \
  (Memory.SRAMSize == 0 ? (uint8_t *)MAP_NONE : (uint8_t *)MAP_RONLY_SRAM)
...
static int32_t ScoreHiROM(bool skip_header, int32_t romoff) {
  ...
  if ((base[0xdc] + (base[0xdd] << 8) + base[0xde] + (base[0xdf] << 8)) ==
      0xffff) {
    score += 2;
...
bool S9xInitMemory(void) {
  ...
  if (!Memory.RAM || !Memory.SRAM || !Memory.VRAM || !Memory.ROM ||
      !Memory.Map || !Memory.MapInfo || !IPPU.ScreenColors || !IPPU.TileCache ||
      !IPPU.TileCached || !bytes0x2000) {
    S9xDeinitMemory();
    return false;
  }
...
bool LoadROM(const char *filename) {
  ...
  Memory.CalculatedSize =
      TotalFileSize & ~0x1FFF; /* round down to lower 0x2000 */
  ...
  if ((Memory.CalculatedSize > 0x400000 &&
       !(Memory.ROM[0x7FD5] == 0x32 &&
         ((Memory.ROM[0x7FD6] & 0xF0) == 0x40)) && /* exclude S-DD1 */
  ...
  Memory.LoROM =
      (Settings.ForceLoROM || (!Settings.ForceHiROM && lo_score >= hi_score));
  Memory.HiROM = !Memory.LoROM;

  /* More */
  if (!Settings.ForceHiROM && !Settings.ForceLoROM &&
      !Settings.ForceInterleaved && !Settings.ForceInterleaved2 &&
      !Settings.ForceNotInterleaved && !Settings.ForceSuperFX &&
      !Settings.ForceNoSuperFX && !Settings.ForceDSP1 &&
      !Settings.ForceNoDSP1 && !Settings.ForceSA1 && !Settings.ForceNoSA1 &&
      !Settings.ForceC4 && !Settings.ForceNoC4 && !Settings.ForceSDD1 &&
      !Settings.ForceNoSDD1) {
    if (strncmp((char *)&Memory.ROM[0x7fc0], "YUYU NO QUIZ DE GO!GO!", 22) ==
            0 ||
        strncmp((char *)&Memory.ROM[0x7fc0], "SP MOMOTAROU DENTETSU2", 22) ==
            0 ||
        strncmp((char *)&Memory.ROM[0x7fc0], "SUPER FORMATION SOCCE", 21) ==
            0) {
      Memory.LoROM = true;
      Memory.HiROM = false;
    }
    /* BS Zooっと麻雀 */
    else if ((strncmp((char *)&Memory.ROM[0xffc0], "Ｚｏｏっと麻雀！", 16) ==
              0) ||
             (strncmp((char *)&Memory.ROM[0xffc0], "Zooっと麻雀!IVT", 15) ==
              0)) {
      Memory.LoROM = false;
      Memory.HiROM = true;
    }
    ...
    else if (strncmp((char *)&Memory.ROM[0xffc0], "BATMAN--REVENGE JOKER",
                     21) == 0) {
      Memory.LoROM = true;
      Memory.HiROM = false;
    }
    /* THE DUEL: TEST DRIVE */
    else if (strncmp((char *)&Memory.ROM[0x7fc0], "THE DUEL: TEST DRIVE", 20) ==
             0) {
      Memory.LoROM = true;
      Memory.HiROM = false;
    }
    /* ポパイ いじわる魔女シーハッグの巻 */
    else if (strncmp((char *)&Memory.ROM[0x7fc0], "POPEYE IJIWARU MAJO", 19) ==
             0) {
      Memory.LoROM = true;
      Memory.HiROM = false;
    }
    /* Pop'nツインビー サンプル版 */
      ...
      Memory.HiROM = false;
               ...
               strncmp((char *)&Memory.ROM[0xffc0], "WWF SUPER WRESTLEMANIA",
                       22) == 0) {
      Memory.LoROM = true;
      Memory.HiROM = false;
    }
...
void InitROM(bool Interleaved) {
  ...
  if (Memory.HiROM) {
    ...
    if (((Memory.ROMSpeed & 0x0F) == 0x0A) &&
        ((Memory.ROMType & 0xF0) == 0xF0)) {
    ...
    if ((Memory.ROMSpeed & ~0x10) == 0x25)
    ...
    else if (strncmp((char *)&Memory.ROM[0x7fc0], "SOUND NOVEL-TCOOL", 17) ==
                 0 ||
             strncmp((char *)&Memory.ROM[0x7fc0], "DERBY STALLION 96", 17) ==
                 0) {
      LoROM24MBSMap();
      ...
      Settings.DSP1Master = false;
               ...
               (strncmp((char *)&Memory.ROM[0x7fc0], "demon's blazon", 14) ==
                0) ||
  ...
  if (Memory.SRAMSize > 5) {
    printf("WARNING: SRAM was reduced to 64K by me. If you see this message, "
           "tell me about it.\n");
    Memory.SRAMSize = 5;
  }

  Memory.SRAMMask =
      Memory.SRAMSize ? ((1 << (Memory.SRAMSize + 3)) * 128) - 1 : 0;
  ...
  */
  ...
  if (Settings.ForceNTSC)
    ...
    Settings.PAL = !(Memory.ROMRegion == 13 || Memory.ROMRegion == 1 ||
                     Memory.ROMRegion == 0);
  ...
  printf("Rom loaded: name: %s, id: %s, company: %s, size: %dKB\n",
         ...
         (int)(Memory.CalculatedSize / 1024));
}
...
void ResetSpeedMap(void) {
  ...
  for (i = 0; i < 0x400; i += 0x10) {
    Memory.MapInfo[i + 2].Speed = Memory.MapInfo[0x800 + i + 2].Speed =
        ONE_CYCLE;
    Memory.MapInfo[i + 3].Speed = Memory.MapInfo[0x800 + i + 3].Speed =
        ONE_CYCLE;
    Memory.MapInfo[i + 4].Speed = Memory.MapInfo[0x800 + i + 4].Speed =
        ONE_CYCLE;
    Memory.MapInfo[i + 5].Speed = Memory.MapInfo[0x800 + i + 5].Speed =
        ONE_CYCLE;
  }
  FixROMSpeed();
}
    ...
    for (c = 0; c < 0x0f; c++) {
      for (i = 0; i < 8; i++) {
        Memory.Map[(c << 4) + 0xF00 + i] = Memory.Map[(c << 4) + 0x700 + i] =
            MAP_LOROM_SRAM_OR_NONE;
        Memory.MapInfo[(c << 4) + 0xF00 + i].Type =
...
void LoROMMap(void) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    ...
    if (Settings.C4) {
      ...
      Memory.Map[c + 6] = Memory.Map[c + 0x806] =
          (uint8_t *)bytes0x2000 - 0x6000;
      Memory.Map[c + 7] = Memory.Map[c + 0x807] =
          (uint8_t *)bytes0x2000 - 0x6000;
    }
  ...
  for (c = 0; c < 0x400; c += 16) {
    for (i = c; i < c + 8; i++)
      Memory.Map[i + 0x400] = Memory.Map[i + 0xc00] =
          &Memory.ROM[(c << 11) % Memory.CalculatedSize];

    for (i = c + 8; i < c + 16; i++)
      Memory.Map[i + 0x400] = Memory.Map[i + 0xc00] =
          &Memory.ROM[((c << 11) + 0x200000) % Memory.CalculatedSize] - 0x8000;

    for (i = c; i < c + 16; i++)
      Memory.MapInfo[i + 0x400].Type = Memory.MapInfo[i + 0xC00].Type =
          MAP_TYPE_ROM;
  }
...
void HiROMMap(void) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    ...
    for (i = c + 8; i < c + 16; i++) {
      Memory.Map[i] = Memory.Map[i + 0x800] =
          &Memory.ROM[(c << 12) % Memory.CalculatedSize];
      Memory.MapInfo[i].Type = Memory.MapInfo[i + 0x800].Type = MAP_TYPE_ROM;
    }
  }
...
void TalesROMMap(bool Interleaved) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    ...
    if (c >= 0x200) {
      ...
      Memory.MapInfo[6 + c].Type = Memory.MapInfo[7 + c].Type =
          Memory.MapInfo[0x806 + c].Type = Memory.MapInfo[0x807 + c].Type =
              MAP_TYPE_RAM;
    } else {
      Memory.Map[c + 6] = Memory.Map[c + 0x806] = (uint8_t *)MAP_NONE;
      Memory.Map[c + 7] = Memory.Map[c + 0x807] = (uint8_t *)MAP_NONE;
    ...
    for (i = c + 8; i < c + 16; i++) {
      Memory.Map[i] =
          &Memory
               .ROM[((c << 12) % (Memory.CalculatedSize - 0x400000)) + OFFSET0];
  ...
  for (c = 0; c < 0x400; c += 16) {
    for (i = c; i < c + 8; i++) {
      Memory.Map[i + 0x400] =
          &Memory
               .ROM[((c << 12) % (Memory.CalculatedSize - 0x400000)) + OFFSET1];
      Memory.Map[i + 0x408] =
          &Memory
               .ROM[((c << 12) % (Memory.CalculatedSize - 0x400000)) + OFFSET1];
  ...
  Memory.ROMChecksum =
      *(Memory.Map[8] + 0xFFDE) + (*(Memory.Map[8] + 0xFFDF) << 8);
  Memory.ROMComplementChecksum =
      *(Memory.Map[8] + 0xFFDC) + (*(Memory.Map[8] + 0xFFDD) << 8);
...
void LoROM24MBSMap(void) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    for (i = c; i < c + 8; i++)
      Memory.Map[i + 0x400] = Memory.Map[i + 0xc00] =
          &Memory.ROM[((c << 11) + 0x200000) % Memory.CalculatedSize];
...
void SRAM1024KLoROMMap(void) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    Memory.Map[c + 0] = Memory.Map[c + 0x800] = Memory.RAM;
    Memory.Map[c + 1] = Memory.Map[c + 0x801] = Memory.RAM;
    Memory.MapInfo[c + 0].Type = Memory.MapInfo[c + 0x800].Type =
        Memory.MapInfo[c + 0x400].Type = Memory.MapInfo[c + 0xc00].Type =
            MAP_TYPE_RAM;
    Memory.MapInfo[c + 1].Type = Memory.MapInfo[c + 0x801].Type =
        Memory.MapInfo[c + 0x401].Type = Memory.MapInfo[c + 0xc01].Type =
            MAP_TYPE_RAM;

    Memory.Map[c + 2] = Memory.Map[c + 0x802] = Memory.Map[c + 0x402] =
        Memory.Map[c + 0xc02] = (uint8_t *)MAP_PPU;
    Memory.Map[c + 3] = Memory.Map[c + 0x803] = Memory.Map[c + 0x403] =
        Memory.Map[c + 0xc03] = (uint8_t *)MAP_PPU;
    Memory.Map[c + 4] = Memory.Map[c + 0x804] = Memory.Map[c + 0x404] =
        Memory.Map[c + 0xc04] = (uint8_t *)MAP_CPU;
    Memory.Map[c + 5] = Memory.Map[c + 0x805] = Memory.Map[c + 0x405] =
        Memory.Map[c + 0xc05] = (uint8_t *)MAP_CPU;
    Memory.Map[c + 6] = Memory.Map[c + 0x806] = Memory.Map[c + 0x406] =
        Memory.Map[c + 0xc06] = (uint8_t *)MAP_NONE;
    Memory.Map[c + 7] = Memory.Map[c + 0x807] = Memory.Map[c + 0x407] =
        Memory.Map[c + 0xc07] = (uint8_t *)MAP_NONE;
    for (i = c + 8; i < c + 16; i++) {
      Memory.Map[i] = Memory.Map[i + 0x800] = Memory.Map[i + 0x400] =
          Memory.Map[i + 0xc00] =
              &Memory.ROM[(c << 11) % Memory.CalculatedSize] - 0x8000;
      Memory.MapInfo[i].Type = Memory.MapInfo[i + 0x800].Type =
          Memory.MapInfo[i + 0x400].Type = Memory.MapInfo[i + 0xC00].Type =
              MAP_TYPE_ROM;
    }
  }
...
void CapcomProtectLoROMMap(void) {
  ...
  /* Banks 00->3f and 80->bf */
  for (c = 0; c < 0x400; c += 16) {
    Memory.Map[c + 0] = Memory.Map[c + 0x800] = Memory.Map[c + 0x400] =
        Memory.Map[c + 0xc00] = Memory.RAM;
    Memory.Map[c + 1] = Memory.Map[c + 0x801] = Memory.Map[c + 0x401] =
        Memory.Map[c + 0xc01] = Memory.RAM;
    Memory.MapInfo[c + 0].Type = Memory.MapInfo[c + 0x800].Type =
        Memory.MapInfo[c + 0x400].Type = Memory.MapInfo[c + 0xc00].Type =
            MAP_TYPE_RAM;
    Memory.MapInfo[c + 1].Type = Memory.MapInfo[c + 0x801].Type =
...
void JumboLoROMMap(bool Interleaved) {
  ...
  for (c = 0; c < 0x400; c += 16) {
    ...
    for (i = c + 8; i < c + 16; i++) {
      Memory.Map[i] =
          &Memory.ROM[((c << 11) % (Memory.CalculatedSize - 0x400000)) +
                      OFFSET0] -
          0x8000;
      Memory.Map[i + 0x800] =
          &Memory.ROM[((c << 11) % (0x400000)) + OFFSET2] - 0x8000;
  ...
  for (c = 0x400; c < 0x800; c += 16) {
    ...
    for (i = c; i < c + 8; i++) {
      Memory.Map[i] =
          &Memory
               .ROM[((c << 11) % (Memory.CalculatedSize - 0x400000)) + OFFSET0];
  ...
  for (k = 0; k < 256; k++) {
    uint8_t *bank =
        0x8000 + Memory.Map[8 + (k << 4)]; /* use upper half of the banks, and
                                              adjust for LoROM. */
    for (l = 0; l < 0x8000; l++)
      sum += bank[l];
  }
...
const char *TVStandard(void) { return Settings.PAL ? "PAL" : "NTSC"; }

const char *Speed(void) { return Memory.ROMSpeed & 0x10 ? "120ns" : "200ns"; }

const char *MapType(void) { return Memory.HiROM ? "HiROM" : "LoROM"; }

const char *ROMID(void) { return Memory.ROMId; }

bool match_na(const char *str) { return strcmp(Memory.ROMName, str) == 0; }
...
void ApplyROMPatches(void) {
  ...
  if ((strncmp((char *)&Memory.ROM[0x7fc0], "MEY Fun with Numbers", 20) == 0)) {
    ...
    for (cvcount = 0; cvcount < 16; cvcount++) {
      memcpy(&Memory.ROM[0x8000 * cvcount],
             &Memory.ROM[0x10000 * cvcount + 0x100000 + 0x8000], 0x8000);
      memcpy(&Memory.ROM[0x8000 * cvcount + 0x80000],
             &Memory.ROM[0x10000 * cvcount + 0x100000], 0x8000);
    }
  }
  ...
  } else if (strncmp("SUPER MARIOWORLD", (char *)&Memory.ROM[0x7FC0], 16) ==
             0) {
    Memory.ROM[0x00006D] = 0x42;
  } else if (strncmp("ALL_STARS + WORLD", (char *)&Memory.ROM[0x7FC0], 17) ==
             0) {
...
void ApplyROMFixes(void) {
  ...
  [14:25:13] <@Nach>     case 0x6810aa95: Bazooka Blitzkreig swapped sizes hack
  -handled [14:25:17] <@Nach>     case 0x61E29C06: The Tick region hack
  [14:25:19] <@Nach>     case 0x1EF90F74: Jikkyou Keiba Simulation Stable Star
  PAL hack [14:25:23] <@Nach>     case 0x4ab225b5: So called Krusty's Super Fun
  House (E) [14:25:25] <@Nach>     case 0x77fd806a: Donkey Kong Country 2 (E)
  v1.1 bad dump -handled [14:25:27] <@Nach>     case 0x340f23e5: Donkey Kong
  Country 3 (U) copier hack - handled
  */
  ...
  if (match_na("EARTHWORM JIM 2") || match_na("PRIMAL RAGE") ||
      match_na("CLAY FIGHTER") || match_na("ClayFighter 2") ||
      strncasecmp(Memory.ROMName, "MADDEN", 6) == 0 ||
      strncmp(Memory.ROMName, "NHL", 3) == 0 || match_na("WeaponLord") ||
  ...
  if (match_id("CQ  ") ||
      /* Illusion of Gaia */
      strncmp(Memory.ROMId, "JG", 2) == 0 || match_na("GAIA GENSOUKI 1 JPN"))
    IAPU.OneCycle = 13;
  ...
  /* RENDERING RANGER R2 */
           ...
           /* Mark Davis */
           strncmp(Memory.ROMName, "THE FISHING MASTER", 18) ==
           ...
           /* Soulblazer */
           match_na("SOULBLAZER - 1 USA") || match_na("SOULBLADER - 1") ||
           /* Terranigma */
           ...
           ((strncmp(Memory.ROMName, "Parlor", 6) == 0 ||
             ...
             strncmp(Memory.ROMName, "SANKYO Fever! \xCC\xA8\xB0\xCA\xDE\xB0!",
                     21) == 0) &&
            strcmp(Memory.CompanyId, "A0") == 0) ||
           match_na("DARK KINGDOM") || match_na("ZAN3 SFC") ||
           match_na("HIOUDEN") ||
           match_na("\xC3\xDD\xBC\xC9\xB3\xC0") || /* Tenshi no Uta */
           match_na("FORTUNE QUEST") || match_na("FISHING TO BASSING") ||
           ...
           strncmp(Memory.ROMName, "LETs PACHINKO(", 14) ==
               0) /* A set of BS games */
    IAPU.OneCycle = 15;
#endif
  ...
  Settings.WinterGold = match_na("FX SKIING NINTENDO 96") ||
                        match_na("DIRT RACER") || Settings.StarfoxHack;
  ...
  if ((match_na("SFX SUPERBUTOUDEN2")) || (match_na("ALIEN vs. PREDATOR")) ||
      (match_na("STONE PROTECTORS")) || (match_na("SUPER BATTLETANK 2")))
  ...
  else if (strncmp(Memory.ROMId, "A3M", 3) == 0 &&
  ...
  else if (match_na("WILD TRAX") || match_na("STAR FOX 2") ||
           match_na("YOSSY'S ISLAND") || match_na("YOSHI'S ISLAND"))
    CPU.TriedInterleavedMode2 = true;
  /* Start Trek: Deep Sleep 9 */
  else if (strncmp(Memory.ROMId, "A9D", 3) == 0 &&
           Settings.CyclesPercentage == 100)
    Settings.H_Max = (SNES_CYCLES_PER_SCANLINE * 110) / 100;
  ...
  /* Gundam Knight Story */
  if (match_na(
          "SFX \xC5\xB2\xC4\xB6\xDE\xDD\xC0\xDE\xD1\xD3\xC9\xB6\xDE\xC0\xD8 "
          "1")) {
