...
int ws_input_poll(int mode) {
  ...
  // Map D-Pad to both X and Y pads for compatibility
...
static void SubmitFrame(void) {
  ...
  int have = apuBufLen();
  if (have > 0) {
    int16_t samples[have * 2]; // 2 channels per frame
    ...
    rg_audio_submit((const rg_audio_frame_t *)samples, have);
  }
}
