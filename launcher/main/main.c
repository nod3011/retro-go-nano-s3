...
static rg_gui_event_t toggle_tabs_cb(rg_gui_option_t *option,
                                     rg_gui_event_t event) {
  if (event == RG_DIALOG_ENTER) {
    ...
    for (size_t i = 0; i < gui.tabs_count; ++i)
      *opt++ =
          (rg_gui_option_t){i, gui.tabs[i]->name, "...", 1, &toggle_tab_cb};
...
static rg_gui_event_t scroll_mode_cb(rg_gui_option_t *option,
                                     rg_gui_event_t event) {
...
static rg_gui_event_t start_screen_cb(rg_gui_option_t *option,
                                      rg_gui_event_t event) {
  const char *modes[START_SCREEN_COUNT] = {_("Auto"), _("Carousel"),
                                           _("Browser")};
...
static rg_gui_event_t show_preview_cb(rg_gui_option_t *option,
                                      rg_gui_event_t event) {
  const char *modes[] = {_("None"), _("Cover,Save"), _("Save,Cover"),
                         _("Cover only"), _("Save only")};
...
static rg_gui_event_t color_theme_cb(rg_gui_option_t *option,
                                     rg_gui_event_t event) {
...
static rg_gui_event_t startup_app_cb(rg_gui_option_t *option,
                                     rg_gui_event_t event) {
...
static rg_gui_event_t local_update_cb(rg_gui_option_t *option,
                                      rg_gui_event_t event) {
  if (event == RG_DIALOG_ENTER) {
    if (rg_system_have_app("updater")) {
      rg_system_switch_app("updater", "System Update", NULL, 0);
      return RG_DIALOG_SELECT;
    } else {
      rg_gui_alert(_("Error"), _("Updater partition not found!"));
    }
  }
  return RG_DIALOG_VOID;
}

static rg_gui_event_t prebuild_cache_cb(rg_gui_option_t *option,
                                        rg_gui_event_t event) {
  if (event == RG_DIALOG_ENTER) {
...
static void try_migrate(void) {
  // A handful of retro-go versions used the weird /odroid/*.txt to store books.
  // Let's move them!
  if (rg_settings_get_number(NS_GLOBAL, "Migration", 0) < 1290) {
#ifdef RG_TARGET_ODROID_GO
    rg_storage_mkdir(RG_BASE_PATH_CONFIG);
    rename(RG_STORAGE_ROOT "/odroid/favorite.txt",
           RG_BASE_PATH_CONFIG "/favorite.txt");
    rename(RG_STORAGE_ROOT "/odroid/recent.txt",
           RG_BASE_PATH_CONFIG "/recent.txt");
#endif
    rg_settings_set_number(NS_GLOBAL, "Migration", 1290);
    rg_settings_commit();
  }

  // Some of our save formats have diverged and cause issue when they're shared
  // with Go-Play
  if (rg_settings_get_number(NS_GLOBAL, "Migration", 0) < 1390) {
#ifdef RG_TARGET_ODROID_GO
    if (rg_storage_exists(RG_STORAGE_ROOT "/odroid/data"))
      rg_gui_alert(
          "Save path changed in 1.32",
          "Save format is no longer fully compatible with Go-Play and can "
          "cause corruption.\n\n"
  ...
  if (rg_settings_get_number(NS_GLOBAL, "Migration", 0) < 1440) {
    // Bit order and default value of the indicators has changed in 1.44, reset
    // it
    rg_settings_set_number(NS_GLOBAL, "Indicators",
                           (1 << RG_INDICATOR_POWER_LOW));
...
static void about_handler(rg_gui_option_t *dest) {
  *dest++ = (rg_gui_option_t){0, _("Build CRC cache"), NULL,
...
void app_main(void) {
  ...
  if (!rg_storage_ready()) {
    ...
    rg_gui_alert(_("SD Card Error"),
                 _("Storage mount failed.\nMake sure the card is FAT32."));
  } else {
...
#ifdef ESP_PLATFORM
  // The launcher makes a lot of small allocations and it sometimes fills
  // internal RAM, causing the SD Card driver to stop working. Lowering
  // CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL and manually using rg_alloc to do
  // internal allocs when needed is a better solution, but that would have to be
  // done for every app. This is a good workaround for now.
