...
static int scan_folder_cb(const rg_scandir_t *entry, void *arg) {
  ...
  char *name = rg_bucket_insert(app->filenames, entry->basename,
...
static void application_init(retro_app_t *app) {
  ...
  rg_storage_scandir(app->paths.roms, scan_folder_cb, app,
                     RG_SCANDIR_RECURSIVE);
  rg_storage_scandir(app->paths.saves, scan_saves_cb, app,
                     RG_SCANDIR_RECURSIVE);
  // rg_storage_scandir(app->paths.covers, scan_folder_cb3, app,
  // RG_SCANDIR_RECURSIVE);
...
static void application_start(retro_file_t *file, int load_state) {
  ...
  bookmark_add(BOOK_TYPE_RECENT,
               file); // This could relocate *file, but we no longer need it
...
static void crc_cache_init(void) {
  ...
  rg_storage_read_file(RG_BASE_PATH_CACHE "/crc32.bin", &data_ptr, &data_len,
                       RG_FILE_USER_BUFFER);
  if (crc_cache->magic == CRC_CACHE_MAGIC &&
      crc_cache->count <= CRC_CACHE_MAX_ENTRIES) {
...
static void crc_cache_save(void) {
  ...
  size_t data_len =
      RG_MIN(8 + (crc_cache->count * sizeof(crc_cache->entries[0])),
             sizeof(*crc_cache));
  crc_cache_dirty = !rg_storage_write_file(RG_BASE_PATH_CACHE "/crc32.bin",
                                           crc_cache, data_len, 0);
}

static uint32_t crc_cache_calc_key(retro_file_t *file) {
  // return ((uint64_t)rg_crc32(0, (void *)file->name, strlen(file->name)) << 33
  // | file->size); This should be reasonably unique
...
void crc_cache_prebuild(void) {
  ...
  for (int i = 0; i < apps_count; i++) {
    ...
    for (int j = 0; j < app->files_count; j++) {
      ...
      rg_gui_draw_message(_("Scanning %s %d/%d"), app->short_name, j,
                          app->files_count);
...
static void tab_refresh(tab_t *tab, const char *selected) {
  ...
  if (items_count == 0) {
    ...
    sprintf(tab->listbox.items[2].text, _("Place roms in folder: %s"),
            rg_relpath(app->paths.roms));
    sprintf(tab->listbox.items[3].text, _("With file extension: %s"),
            app->extensions);
    ...
    for (int i = 0; i < tab->listbox.length; i++) {
      ...
      if (file &&
          strcmp(file->name, selected) == 0) // file->folder == selected->folder
      {
...
static void event_handler(gui_event_t event, tab_t *tab) {
  ...
  if (event == TAB_INIT) {
    ...
    {
      // rg_storage_exists can take a long time on large folders (200ms), this
      // is much faster
      for (size_t i = 0; i < app->files_count; ++i) {
        retro_file_t *file = &app->files[i];
        if (selected->folder == file->folder &&
            strcmp(selected->name, file->name) == 0) {
...
static void application(const char *desc, const char *name, const char *exts,
                        const char *part, uint16_t crc_offset) {
...
void applications_init(void) {
  application("Nintendo Entertainment System", "nes", "nes fc fds nsf zip",
              "retro-core", 16);
  ...
  application("Nintendo Gameboy Advance", "gbsp", "gba zip", "gbsp", 0);
