...
BYTE **ROMMap;       // C-ROMoN}bv
int ROMBanks;        // C-ROMoN
BYTE **RAMMap;       // C-RAMoN}bv
int RAMBanks;        // C-RAMoN
int RAMSize;         // C-RAMe
WORD IEep[64];       // EEPROM
struct EEPROM sIEep; // EEPROMǂݏp\́ij
struct EEPROM sCEep; // EEPROMǂݏp\́iJ[gbWj
int CartKind;        // Z[u̎ށiCK_EEP = EEPROMj
...
static WORD DefColor[] = {MONO(0xF), MONO(0xE), MONO(0xD), MONO(0xC),
                          MONO(0xB), MONO(0xA), MONO(0x9), MONO(0x8),
...
void ComEEP(struct EEPROM *eeprom, WORD *cmd, WORD *data) {
  ...
  case 1: //
    if (eeprom->we) {
      ...
      if (ROMBanks == 1 && addr == 0x3A) // AiUwuł
      {
        WsSplash();
        Run = 0; // p[\if[^Ō݂̏Ȃ̂ŏI
      }
...
inline BYTE ReadMem(DWORD A) { return Page[(A >> 16) & 0xF][A & 0xFFFF]; }

void WriteMem(DWORD A, BYTE V) { (*WriteMemFnTable[(A >> 16) & 0x0F])(A, V); }
...
static void WriteCRam(DWORD A, BYTE V) {
  ...
  if (RAMSize != 0x40000 || BNK1SEL < 8) {
    ...
    // FLASH ROM use SRAM bank(port 0xC1:8-14)(0xC1:15 0xF0000-0xFFFFF are write
    // protected)
...
void WriteIO(DWORD A, BYTE V) {
  ...
  case 0xB5:
    KEYCTL = (BYTE)(V & 0xF0);
    if (KEYCTL & 0x40)
      KEYCTL |= (BYTE)((ButtonState >> 8) & 0x0F);
    if (KEYCTL & 0x20)
      KEYCTL |= (BYTE)((ButtonState >> 4) & 0x0F);
    if (KEYCTL & 0x10)
      KEYCTL |= (BYTE)(ButtonState & 0x0F);
    return;
  case 0xB6:
    IRQACK &= (BYTE)~V;
    return;
...
WriteMemFn WriteMemFnTable[16] = {
    WriteIRam, WriteCRam, WriteRom, WriteRom, WriteRom, WriteRom,
    WriteRom,  WriteRom,  WriteRom, WriteRom, WriteRom, WriteRom,
    WriteRom,  WriteRom,  WriteRom, WriteRom,
};
...
void WsRomPatch(BYTE *buf) {
  if ((buf[0] == 0x01) && (buf[1] == 0x01) &&
      (buf[2] == 0x16)) // SWJ-BANC16 STAR HEARTS
  {
    RAMBanks = 1;
    RAMSize = 0x8000;
    CartKind = 0;
  ...
  if ((buf[0] == 0x01) && (buf[1] == 0x00) &&
      (buf[2] == 0x2C || buf[2] == 0x2F)) // SWJ-BAN02C,02F fW^p[gi[
  {
    RAMBanks = 1;
    RAMSize = 0x8000;
    CartKind = 0;
  ...
  if ((buf[0] == 0x01) && (buf[1] == 0x01) &&
      (buf[2] == 0x38)) // SWJ-BANC38 NARUTO ؃mtE@
  {
    RAMBanks = 1;
    RAMSize = 0x10000;
    CartKind = 0;
...
void WsSplash(void) {
  ...
  const unsigned char *pdataFontArray = pdata + 169 * 16 + 13;
  const unsigned char *pdataFont = pdata + 175 * 16 + 5;
...
void WsCpyPdata(BYTE *dst) { memcpy(dst, pdata, size_pdata); }

void Sleep(int ticks) {
  int64_t ini = rg_system_timer();
  while (rg_system_timer() - ini < (ticks * 10000))
    ;
}

void SetHVMode(int Mode) { HVMode = Mode; }
