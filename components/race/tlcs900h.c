...
//
//  010906 - Checked and fixed ADD/ADC/SUB/SBC/AND/OR/XOR/MUL/MULS/DAA for 100%
//  correctness 010324 - Fixed a stupid flag bug in the rotate & shift
//  instructions 001230 - Fixed and cleaned XORCF and ORCF instructions
// 991225 - Added the previous register bank
//  991219 - Finished redoing all the flag handling, time to do some bugfixing
//  again... 991215 - Hopefully fixed the V flag in the ADD, ADC, SUB and SBC
//  operations 991210 - Fixed some minor bugs after a cdoe read 991207 - Fixed a
//  bug in DIVS and a bug in the bit test operations 991127 - Redid the signed
//  conditionals, hopefully good this time
//
// TODO:
...
//
//
// there don't seem to be any direct calls into the bios. The way bios calls
// seem to be implemented is through SWI 1. SWI 1 is called and some parameters
// are passed in some registers.
//
//
...
//
// the encoding of general 8/16/32 bit registers is unknown at this point; ie,
// which register number refers to which register (part) name.
...
//
// Also some special additional registers from this particular member of the
// 900H seems to be getting used, presumably for communication purposes???
//
...
// PC,   SR,   XSSP, XNSP     20,21,22,23 XSSP = System Stack Pointer, XSP =
// current stack pointer
// unsigned int gen_regs[24];
unsigned int gen_regsXWA0, gen_regsXBC0, gen_regsXDE0, gen_regsXHL0,
    gen_regsXWA1, gen_regsXBC1, gen_regsXDE1, gen_regsXHL1, gen_regsXWA2,
    gen_regsXBC2, gen_regsXDE2, gen_regsXHL2, gen_regsXWA3, gen_regsXBC3,
    gen_regsXDE3, gen_regsXHL3, gen_regsXIX, gen_regsXIY, gen_regsXIZ,
...
// number of states passed and the number of states until a new interrupt should
// occur 1 state = 100ns at 20 MHz 1 state =  80ns at 25 MHz
int state;
int checkstate;
int DMAstate;
...
// used during decoding of instructions, will hold the arguments for the
// operands
unsigned int mem;
...
// lastbyte - to keep track of the last read byte; used when extra information
// is stored in the
//            opcode itself.
#ifndef TARGET_GP2X
...
static INLINE void tlcsFastMemWriteB(unsigned int addr, unsigned char data) {
  // Flavor:  I think we're getting carried away with this addr&0xFFFFFF junk.
  // It's all over, now.  :(
...
static INLINE void tlcsMemWriteBaddrB(unsigned char addr, unsigned char data) {
  ...
  case 0xA2: // L CH DAC Control Register
    ...
    break;
    /*case 0xA3:	// R CH DAC Control Register  //Flavor hack for mono
      only sound ngpSoundExecute(); if (cpuram[0xB8] == 0xAA)
...
#define cond0() (FALSE)
#define cond1()                                                                \
  (((gen_regsSR & (SF | VF)) == SF) || ((gen_regsSR & (SF | VF)) == VF))
#define cond2()                                                                \
  ((gen_regsSR & ZF) ||                                                        \
   (((gen_regsSR & (SF | VF)) == SF) || ((gen_regsSR & (SF | VF)) == VF)))
...
static INLINE unsigned char srLT(void) {
  return ((((gen_regsSR & (SF | VF)) == SF) || ((gen_regsSR & (SF | VF)) == VF))
              ? 1
              : 0);
}
...
static INLINE unsigned char srOV(void) { return ((gen_regsSR & VF) ? 1 : 0); }
static INLINE unsigned char srMI(void) { return ((gen_regsSR & SF) ? 1 : 0); }
static INLINE unsigned char srZ(void) { return ((gen_regsSR & ZF) ? 1 : 0); }
static INLINE unsigned char srC(void) { return ((gen_regsSR & CF) ? 1 : 0); }
...
static INLINE unsigned char srNOV(void) { return ((gen_regsSR & VF) ? 0 : 1); }
static INLINE unsigned char srPL(void) { return ((gen_regsSR & SF) ? 0 : 1); }
static INLINE unsigned char srNZ(void) { return ((gen_regsSR & ZF) ? 0 : 1); }
static INLINE unsigned char srNC(void) { return ((gen_regsSR & CF) ? 0 : 1); }
...
#define NG_PUSH_BYTE(src, s)                                                   \
  tlcsFastMemWriteB(--gen_regsXSP, src);                                       \
  return s;
#define NG_PUSH_WORD(src, s)                                                   \
  tlcsFastMemWriteW(gen_regsXSP -= 2, src);                                    \
  return s;
#define NG_PUSH_LONG(src, s)                                                   \
  tlcsFastMemWriteL(gen_regsXSP -= 4, src);                                    \
  return s;

int pushsr(void) { NG_PUSH_WORD((unsigned short)gen_regsSR & 0x0000ffff, 4); }
int pushF(void) { NG_PUSH_BYTE((unsigned char)gen_regsSR & 0x000000ff, 3); }
int pushA(void) { NG_PUSH_BYTE(*cregsB[1], 3); }
int pushRW(void) { NG_PUSH_WORD(*cregsW[opcode & 7], 3); }
int pushRL(void) { NG_PUSH_LONG(*cregsL[opcode & 7], 5); }
int pushrB(void) { NG_PUSH_BYTE(*regB, 5); }
int pushrW(void) { NG_PUSH_WORD(*regW, 5); }
int pushrL(void) { NG_PUSH_LONG(*regL, 7); }
int pushI(void) { NG_PUSH_BYTE(readbyte(), 4); }
int pushwI(void) { NG_PUSH_WORD(readword(), 5); }
int pushM00(void) { NG_PUSH_BYTE(memB, 7); }
int pushwM10(void) { NG_PUSH_WORD(memW, 7); }
...
int ldar(void) // LDAR R,$+4+d16 11110011 00010011 xxxxxxxx xxxxxxxx 001s0RRR
{
  ...
  if (j & 0x10) { // long
    ...
    *cregsW[j & 7] =
        (unsigned short)((gen_regsPC - 1 + (signed short)i) & 0x0000ffff);
  }
  return 11;
}
...
int cpiB(void) // CPI A,(R+)   10000RRR 00010100
{
  ...
  *cregsW[1] -= 1; // BC
  ...
  gen_regsSR =
      gen_regsSR | (i & SF) | ((i) ? NF : NF | ZF) | ((*cregsW[1]) ? VF : 0);
  return 8;
}
...
int cpiW(void) // CPI WA,(R+)   10010RRR 00010100
{
  ...
  *cregsW[1] -= 1; // BC
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF);
  gen_regsSR = gen_regsSR | ((i >> 8) & SF) | ((i) ? NF : NF | ZF) |
               ((*cregsW[1]) ? VF : 0);
  return 8;
}
...
int cpirB(void) // CPIR A,(R+)   10000RRR 00010101
{
  ...
  *cregsW[1] -= 1; // BC
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF);
  gen_regsSR = gen_regsSR | (i & SF) | NF | Ztable[i] | //((i) ? NF : NF|ZF) |
               ((*cregsW[1]) ? VF : 0);
...
static INLINE unsigned char MyAddB(unsigned char i, unsigned char j) {
  ...
  gen_regsSR = gen_regsSR | (i & SF) | ((i) ? 0 : ZF) |
               (((oldi ^ j) ^ i) & HF) |
               (((i ^ oldi) & (i ^ j) & 0x80) ? VF : 0) | ((i < oldi) ? CF : 0);
  return i;
}
...
static INLINE unsigned short MyAddW(unsigned short i, unsigned short j) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR =
      gen_regsSR | ((i >> 8) & SF) | ((i) ? 0 : ZF) | (((oldi ^ j) ^ i) & HF) |
      (((i ^ oldi) & (i ^ j) & 0x8000) ? VF : 0) | ((i < oldi) ? CF : 0);
  return i;
}
...
static INLINE unsigned int MyAddL(unsigned int i, unsigned int j) {
  ...
  i += j;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 24) & SF) | ((i) ? 0 : ZF) |
...
static INLINE unsigned short MyAdcW(unsigned short i, unsigned short j) {
  ...
  i += j + oldC;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 8) & SF) | ((i) ? 0 : ZF) |
               (((oldi ^ j) ^ i) & HF) |
...
static INLINE unsigned char MySubB(unsigned char i, unsigned char j) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR =
      gen_regsSR | (i & SF) | ((i) ? NF : NF | ZF) | (((oldi ^ j) ^ i) & HF) |
      (((j ^ oldi) & (i ^ oldi) & 0x80) ? VF : 0) | ((i > oldi) ? CF : 0);
  return i;
#endif
}
...
static INLINE unsigned int MySubL(unsigned int i, unsigned int j) {
  ...
  gen_regsSR &= ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR |= ((i >> 24) & SF) |
                (Ztable[(i >> 24) & 0xFF] & Ztable[(i >> 16) & 0xFF] &
                 Ztable[(i >> 8) & 0xFF] & Ztable[i & 0xFF]) |
  ...
  i -= j;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 24) & SF) | ((i) ? NF : NF | ZF) |
               (((j ^ oldi) & (i ^ oldi) & 0x80000000) ? VF : 0) |
               ((i > oldi) ? CF : 0);
  return i;
#endif
...
static INLINE unsigned char MySbcB(unsigned char i, unsigned char j) {
  ...
  i = i - j - oldC;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | SZtable[i & 0xFF] | NF | (((oldi ^ j) ^ i) & HF) |
...
#ifdef USE_PARITY_TABLE // speed hack
  gen_regsSR =
      (gen_regsSR & ~VF) | (parityVtable[j >> 8] ^ parityVtable[j & 0xFF]);
#else
  unsigned char k = 0, i;
...
static INLINE void parityL(unsigned int j) {
  ...
  gen_regsSR =
      (gen_regsSR & ~VF) |
      (parityVtable[(j >> 24) & 0xFF] ^ parityVtable[(j >> 16) & 0xFF] ^
       parityVtable[(j >> 8) & 0xFF] ^ parityVtable[j & 0xFF]);
#else
  unsigned char k = 0, i;
...
int daar(void) // DAA r    11001rrr 00010000
{
  ...
  if (gen_regsSR & NF) { // adjust after SUB, SBC or NEG operation
    *regB = oldB - addVal;
  ...
  gen_regsSR = gen_regsSR | (*regB & SF) | ((*regB) ? 0 : ZF) |
               (((oldB ^ addVal) ^ *regB) & HF)
#ifdef USE_PARITY_TABLE
               | parityVtable[*regB];
#else
...
{
  *cregsW[(lastbyte & 7) >> 1] =
      (signed char)(*cregsB[lastbyte & 7]) * (signed char)(*regB);
  return 18;
}
...
{
  *cregsL[lastbyte & 7] =
      (signed short)(*cregsW[lastbyte & 7]) * (signed short)(*regW);
  return 26;
}
...
int mulsrIB(void) // MULS rr,#    11001rrr 00001001 xxxxxxxx
{
  if (opcode >= 0xc8) {
    *(cregsW[(opcode & 7) >> 1]) =
        (signed char)(*cregsB[opcode & 7]) * (signed char)readbyte();
  } else {
...
{
  *cregsW[(lastbyte & 7) >> 1] =
      (signed char)(*cregsB[lastbyte & 7]) * (signed char)memB;
  return 18;
}
...
{
  *cregsL[lastbyte & 7] =
      (signed short)(*cregsW[lastbyte & 7]) * (signed short)memW;
  return 26;
}
...
static INLINE unsigned short myDivB(unsigned short i, unsigned char j) {
  ...
  if (res.quot > 0xFF)
    ...
    gen_regsSR &= ~VF;
  return ((unsigned short)(res.quot & 0xFF)) |
         ((unsigned short)((res.rem & 0xFF) << 8));
}
...
{
  *cregsW[(lastbyte & 7) >> 1] = myDivsB(
      (signed short)(*cregsW[(lastbyte & 7) >> 1]), (signed char)(*regB));
  return 24;
}
...
{
  *cregsL[lastbyte & 7] =
      myDivsW((signed int)(*cregsL[lastbyte & 7]), (signed short)(*regW));
  return 32;
}
...
int divsrIB(void) // DIVS rr,#    11001rrr 00001011 xxxxxxxx
{
  ...
  if (opcode >= 0xc8) {
    *cregsW[(opcode & 7) >> 1] =
        myDivsB((signed short)(*cregsW[(opcode & 7) >> 1]), i);
  } else {
...
int divsRMB00(void) // DIVS RR,(mem)  10000mmm 01011RRR
{
  ...
  *cregsW[(lastbyte & 7) >> 1] =
      myDivsB((signed short)(*cregsW[(lastbyte & 7) >> 1]), i);
  return 24;
}
...
int mular(void) // MULA rr     11011rrr 00011001
{
  ...
  p = cregsL[opcode & 7];
  *p += (signed int)(((signed short)mem_readW(*cregsL[2])) *
                     ((signed short)mem_readW(*cregsL[3])));
  *cregsL[3] -= 2; // XHL
  gen_regsSR = gen_regsSR & ~(SF | ZF | VF);
  gen_regsSR = gen_regsSR | ((*p >> 24) & SF) | ((*p) ? 0 : ZF);
  return 31;
}
...
unsigned char MyAndB(unsigned char i, unsigned char j) {
  ...
  i &= j;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | (i & SF) | ((i) ? HF : HF | ZF)
#ifdef USE_PARITY_TABLE
               | parityVtable[i];
#else
      ;
...
unsigned short MyAndW(unsigned short i, unsigned short j) {
  i &= j;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 8) & SF) | ((i) ? HF : HF | ZF);
  parityW(i);
  return i;
}
...
unsigned int MyAndL(unsigned int i, unsigned int j) {
  i &= j;
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 24) & SF) | ((i) ? HF : HF | ZF);
  return i;
}
...
#define BIT(A, B)                                                              \
  gen_regsSR = (gen_regsSR & ~(ZF | NF)) | ((A & power2[B]) ? HF : HF | ZF);
...
int tset4rB(void) // TSET #4,r   11001rrr 00110100 0000xxxx
{
  ...
  gen_regsSR = gen_regsSR | ((*regB & power2[i]) ? HF : HF | ZF);
  *regB |= power2[i];
  return 6;
}
...
int tset4rW(void) // TSET #4,r   11011rrr 00110100 0000xxxx
{
  ...
  gen_regsSR = gen_regsSR & ~(ZF | NF);
  gen_regsSR = gen_regsSR | ((*regW & power2[i]) ? HF : HF | ZF);
  *regW |= power2[i];
  return 6;
}
...
int tset3M30(void) // TSET #3,(mem)  10110mmm 10101xxx
{
  ...
  gen_regsSR = gen_regsSR & ~(ZF | NF);
  gen_regsSR = gen_regsSR | ((i & power2[i]) ? HF : HF | ZF);
...
int ldx(
    void) // LDX (#8),#   11110111 00000000 xxxxxxxx 00000000 xxxxxxxx 00000000
{
...
static INLINE unsigned char MyRlcB(unsigned char i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | (i & SF) | ((i) ? 0 : ZF) | (i & CF)
#ifdef USE_PARITY_TABLE
               | parityVtable[i];
#else
      ;
...
static INLINE unsigned short MyRlcW(unsigned short i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 8) & SF) | ((i) ? 0 : ZF) | (i & CF);
  parityW(i);
  return i;
}
...
static INLINE unsigned int MyRlcL(unsigned int i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i >> 24) & SF) | ((i) ? 0 : ZF) | (i & CF);
  parityL(i);
  return i;
}
...
unsigned char MyRrcB(unsigned char i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | ((i & SF) ? SF | CF : 0) | ((i) ? 0 : ZF)
#ifdef USE_PARITY_TABLE
               | parityVtable[i];
#else
      ;
...
unsigned short MyRrcW(unsigned short i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | (((i >> 8) & 0x80) ? SF | CF : 0) | ((i) ? 0 : ZF);
  parityW(i);
  return i;
}
...
unsigned int MyRrcL(unsigned int i, unsigned char nr) {
  ...
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | (((i >> 24) & 0x80) ? SF | CF : 0) | ((i) ? 0 : ZF);
  parityL(i);
  return i;
}
...
int rld00(void) // RLD A,(mem)   10000mmm 00000110
{
  ...
  *cregsB[1] = ((*cregsB[1]) & 0xf0) | ((i & 0xf0) >> 4);
  ...
  // update flags
  gen_regsSR = gen_regsSR & ~(SF | ZF | HF | VF | NF | CF);
  gen_regsSR = gen_regsSR | (*cregsB[1] & SF) | ((*cregsB[1]) ? 0 : ZF)
#ifdef USE_PARITY_TABLE
...
static INLINE int VECT_SHUTDOWN(void) { return 10; }
...
/*#define PSP_SYSTEMPARAM_ID_INT_TIMEZONE         6
        ...
        if(!tzInit)
        {
                int tzOffset = 0;
                tzInit=1;
                sceUtilityGetSystemParamInt (PSP_SYSTEMPARAM_ID_INT_TIMEZONE,
&tzOffset); int tzOffsetAbs = tzOffset < 0 ? -tzOffset : tzOffset; int hours =
tzOffsetAbs / 60; int minutes = tzOffsetAbs - hours * 60; static char tz[10];
                sprintf (tz, "GMT%s%02i:%02i", tzOffset < 0 ? "+" : "-", hours,
minutes); setenv ("TZ", tz, 1); tzset ();
        }
#else
...
static INLINE int VECT_ALARMSET(unsigned char day, unsigned char hour,
                                unsigned char minute, unsigned char *result) {
  *result = 0; // SYS_SUCCESS
  return 100;
}

static INLINE int VECT_FLASHWRITE(unsigned char chip, unsigned short size,
...
static INLINE int VECT_FLASHERS(unsigned char chip, unsigned char blockNum,
                                unsigned char *result) {
...
static INLINE int doBios(unsigned char biosNr) {
  ...
  case 0x06: // store something in saveram
    ...
    return VECT_FLASHWRITE(*allregsB[0x30], *allregsW[0x34], *allregsL[0x3C],
                           *allregsL[0x38], allregsB[0x30]);
  ...
  case 0x09:
    return VECT_ALARMSET(*allregsB[0x36], *allregsB[0x35], *allregsB[0x34],
                         allregsB[0x30]);
  case 0x0e:
    return VECT_GEMODESET(
        0x10); // Implementation wrong in ngpc bios???, always set to color
  case 0x14:
    *cregsB[1] = 1; // ld a,01
    break;
  case 0x17:
...
#pragma GCC diagnostic ignored "-Wignored-qualifiers"
#pragma GCC diagnostic ignored "-Wdiscarded-qualifiers"
#define DECODE_TABLE const

// instructions where the highest bit of the first byte is set to 1
// x0000xxx xxxxxxxx
DECODE_TABLE int (*decode_table80[256])() = {
    udef, udef, udef, udef, pushM00, udef, rld00, rrd00, udef, udef, udef, udef,
    udef, udef, udef, udef, ldi, ldir, ldd, lddr, cpiB, cpirB, cpdB, cpdrB,
    udef, ld16M00, udef, udef, udef, udef, udef, udef, ldRM00, ldRM00, ldRM00,
    ldRM00, ldRM00, ldRM00, ldRM00, ldRM00, udef, udef, udef, udef, udef, udef,
    udef, udef, exMRB00, exMRB00, exMRB00, exMRB00, exMRB00, exMRB00, exMRB00,
    exMRB00, addMI00, adcMI00, subMI00, sbcMI00, andMI00, xorMI00, orMI00,
    cpMI00,
    //
    mulRMB00, mulRMB00, mulRMB00, mulRMB00, mulRMB00, mulRMB00, mulRMB00,
    mulRMB00, mulsRMB00, mulsRMB00, mulsRMB00, mulsRMB00, mulsRMB00, mulsRMB00,
    mulsRMB00, mulsRMB00, divRMB00, divRMB00, divRMB00, divRMB00, divRMB00,
    divRMB00, divRMB00, divRMB00, divsRMB00, divsRMB00, divsRMB00, divsRMB00,
    divsRMB00, divsRMB00, divsRMB00, divsRMB00, inc3M00, inc3M00, inc3M00,
    inc3M00, inc3M00, inc3M00, inc3M00, inc3M00, dec3M00, dec3M00, dec3M00,
    dec3M00, dec3M00, dec3M00, dec3M00, dec3M00, udef, udef, udef, udef, udef,
    udef, udef, udef, rlcM00, rrcM00, rlM00, rrM00, slaM00, sraM00, sllM00,
    srlM00,
    //
    addRMB00, addRMB00, addRMB00, addRMB00, addRMB00, addRMB00, addRMB00,
    addRMB00, addMRB00, addMRB00, addMRB00, addMRB00, addMRB00, addMRB00,
    addMRB00, addMRB00, adcRMB00, adcRMB00, adcRMB00, adcRMB00, adcRMB00,
    adcRMB00, adcRMB00, adcRMB00, adcMRB00, adcMRB00, adcMRB00, adcMRB00,
    adcMRB00, adcMRB00, adcMRB00, adcMRB00, subRMB00, subRMB00, subRMB00,
    subRMB00, subRMB00, subRMB00, subRMB00, subRMB00, subMRB00, subMRB00,
    subMRB00, subMRB00, subMRB00, subMRB00, subMRB00, subMRB00, sbcRMB00,
    sbcRMB00, sbcRMB00, sbcRMB00, sbcRMB00, sbcRMB00, sbcRMB00, sbcRMB00,
    sbcMRB00, sbcMRB00, sbcMRB00, sbcMRB00, sbcMRB00, sbcMRB00, sbcMRB00,
    sbcMRB00,
    //
    andRMB00, andRMB00, andRMB00, andRMB00, andRMB00, andRMB00, andRMB00,
    andRMB00, andMRB00, andMRB00, andMRB00, andMRB00, andMRB00, andMRB00,
    andMRB00, andMRB00, xorRMB00, xorRMB00, xorRMB00, xorRMB00, xorRMB00,
    xorRMB00, xorRMB00, xorRMB00, xorMRB00, xorMRB00, xorMRB00, xorMRB00,
    xorMRB00, xorMRB00, xorMRB00, xorMRB00, orRMB00, orRMB00, orRMB00, orRMB00,
    orRMB00, orRMB00, orRMB00, orRMB00, orMRB00, orMRB00, orMRB00, orMRB00,
    orMRB00, orMRB00, orMRB00, orMRB00, cpRMB00, cpRMB00, cpRMB00, cpRMB00,
    cpRMB00, cpRMB00, cpRMB00, cpRMB00, cpMRB00, cpMRB00, cpMRB00, cpMRB00,
    cpMRB00, cpMRB00, cpMRB00, cpMRB00};
// x0010xxx xxxxxxxx
DECODE_TABLE int (*decode_table90[256])() = {
    udef, udef, udef, udef, pushwM10, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, ldiw, ldirw, lddw, lddrw, cpiW, cpirW, cpdW, cpdrW,
    udef, ldw16M10, udef, udef, udef, udef, udef, udef, ldRM10, ldRM10, ldRM10,
    ldRM10, ldRM10, ldRM10, ldRM10, ldRM10, udef, udef, udef, udef, udef, udef,
    udef, udef, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10,
    exMRW10, addwMI10, adcwMI10, subwMI10, sbcwMI10, andwMI10, xorwMI10,
    orwMI10, cpwMI10,
    //
    mulRMW10, mulRMW10, mulRMW10, mulRMW10, mulRMW10, mulRMW10, mulRMW10,
    mulRMW10, mulsRMW10, mulsRMW10, mulsRMW10, mulsRMW10, mulsRMW10, mulsRMW10,
    mulsRMW10, mulsRMW10, divRMW10, divRMW10, divRMW10, divRMW10, divRMW10,
    divRMW10, divRMW10, divRMW10, divsRMW10, divsRMW10, divsRMW10, divsRMW10,
    divsRMW10, divsRMW10, divsRMW10, divsRMW10, incw3M10, incw3M10, incw3M10,
    incw3M10, incw3M10, incw3M10, incw3M10, incw3M10, decw3M10, decw3M10,
    decw3M10, decw3M10, decw3M10, decw3M10, decw3M10, decw3M10, udef, udef,
    udef, udef, udef, udef, udef, udef, rlcwM10, rrcwM10, rlwM10, rrwM10,
    slawM10, srawM10, sllwM10, srlwM10,
    //
    addRMW10, addRMW10, addRMW10, addRMW10, addRMW10, addRMW10, addRMW10,
    addRMW10, addMRW10, addMRW10, addMRW10, addMRW10, addMRW10, addMRW10,
    addMRW10, addMRW10, adcRMW10, adcRMW10, adcRMW10, adcRMW10, adcRMW10,
    adcRMW10, adcRMW10, adcRMW10, adcMRW10, adcMRW10, adcMRW10, adcMRW10,
    adcMRW10, adcMRW10, adcMRW10, adcMRW10, subRMW10, subRMW10, subRMW10,
    subRMW10, subRMW10, subRMW10, subRMW10, subRMW10, subMRW10, subMRW10,
    subMRW10, subMRW10, subMRW10, subMRW10, subMRW10, subMRW10, sbcRMW10,
    sbcRMW10, sbcRMW10, sbcRMW10, sbcRMW10, sbcRMW10, sbcRMW10, sbcRMW10,
    sbcMRW10, sbcMRW10, sbcMRW10, sbcMRW10, sbcMRW10, sbcMRW10, sbcMRW10,
    sbcMRW10,
    //
    andRMW10, andRMW10, andRMW10, andRMW10, andRMW10, andRMW10, andRMW10,
    andRMW10, andMRW10, andMRW10, andMRW10, andMRW10, andMRW10, andMRW10,
    andMRW10, andMRW10, xorRMW10, xorRMW10, xorRMW10, xorRMW10, xorRMW10,
    xorRMW10, xorRMW10, xorRMW10, xorMRW10, xorMRW10, xorMRW10, xorMRW10,
    xorMRW10, xorMRW10, xorMRW10, xorMRW10, orRMW10, orRMW10, orRMW10, orRMW10,
    orRMW10, orRMW10, orRMW10, orRMW10, orMRW10, orMRW10, orMRW10, orMRW10,
    orMRW10, orMRW10, orMRW10, orMRW10, cpRMW10, cpRMW10, cpRMW10, cpRMW10,
    cpRMW10, cpRMW10, cpRMW10, cpRMW10, cpMRW10, cpMRW10, cpMRW10, cpMRW10,
    cpMRW10, cpMRW10, cpMRW10, cpMRW10};
// x0011xxx xxxxxxxx
DECODE_TABLE int (*decode_table98[256])() = {
    udef, udef, udef, udef, pushwM10, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, ldw16M10, udef, udef, udef, udef, udef, udef, ldRM10, ldRM10, ldRM10,
    ldRM10, ldRM10, ldRM10, ldRM10, ldRM10, udef, udef, udef, udef, udef, udef,
    udef, udef, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10, exMRW10,
    exMRW10, addwMI10, adcwMI10, subwMI10, sbcwMI10, andwMI10, xorwMI10,
...
DECODE_TABLE int (*decode_tableA0[256])() = {
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, ldRM20, ldRM20, ldRM20,
    ldRM20, ldRM20, ldRM20, ldRM20, ldRM20, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef,
    //
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    ...
    udef, udef, udef, udef,
    //
    addRML20, addRML20, addRML20, addRML20, addRML20, addRML20, addRML20,
    addRML20, addMRL20, addMRL20, addMRL20, addMRL20, addMRL20, addMRL20,
    addMRL20, addMRL20, adcRML20, adcRML20, adcRML20, adcRML20, adcRML20,
    adcRML20, adcRML20, adcRML20, adcMRL20, adcMRL20, adcMRL20, adcMRL20,
    adcMRL20, adcMRL20, adcMRL20, adcMRL20, subRML20, subRML20, subRML20,
    subRML20, subRML20, subRML20, subRML20, subRML20, subMRL20, subMRL20,
    subMRL20, subMRL20, subMRL20, subMRL20, subMRL20, subMRL20, sbcRML20,
    sbcRML20, sbcRML20, sbcRML20, sbcRML20, sbcRML20, sbcRML20, sbcRML20,
    sbcMRL20, sbcMRL20, sbcMRL20, sbcMRL20, sbcMRL20, sbcMRL20, sbcMRL20,
    sbcMRL20,
    //
    andRML20, andRML20, andRML20, andRML20, andRML20, andRML20, andRML20,
    andRML20, andMRL20, andMRL20, andMRL20, andMRL20, andMRL20, andMRL20,
    andMRL20, andMRL20, xorRML20, xorRML20, xorRML20, xorRML20, xorRML20,
    xorRML20, xorRML20, xorRML20, xorMRL20, xorMRL20, xorMRL20, xorMRL20,
    xorMRL20, xorMRL20, xorMRL20, xorMRL20, orRML20, orRML20, orRML20, orRML20,
    orRML20, orRML20, orRML20, orRML20, orMRL20, orMRL20, orMRL20, orMRL20,
    orMRL20, orMRL20, orMRL20, orMRL20, cpRML20, cpRML20, cpRML20, cpRML20,
    cpRML20, cpRML20, cpRML20, cpRML20, cpMRL20, cpMRL20, cpMRL20, cpMRL20,
    cpMRL20, cpMRL20, cpMRL20, cpMRL20};
// x0110xxx xxxxxxxx
DECODE_TABLE int (*decode_tableB0[256])() = {
    ldMI30, udef, ldwMI30, udef, popM30, udef, popwM30, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, ldM1630, udef,
    ldwM1630, udef, udef, udef, udef, udef, udef, udef, udef, udef, ldaRMW30,
    ldaRMW30, ldaRMW30, ldaRMW30, ldaRMW30, ldaRMW30, ldaRMW30, ldaRMW30,
    andcfAM30, orcfAM30, xorcfAM30, ldcfAM30, stcfAM30, udef, udef, udef,
    ldaRML30, ldaRML30, ldaRML30, ldaRML30, ldaRML30, ldaRML30, ldaRML30,
    ldaRML30, udef, udef, udef, udef, udef, udef, udef, udef,
    //
    ldMR30B, ldMR30B, ldMR30B, ldMR30B, ldMR30B, ldMR30B, ldMR30B, ldMR30B,
    udef, udef, udef, udef, udef, udef, udef, udef, ldMR30W, ldMR30W, ldMR30W,
    ldMR30W, ldMR30W, ldMR30W, ldMR30W, ldMR30W, udef, udef, udef, udef, udef,
    udef, udef, udef, ldMR30L, ldMR30L, ldMR30L, ldMR30L, ldMR30L, ldMR30L,
    ldMR30L, ldMR30L, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef,
    //
    andcf3M30, andcf3M30, andcf3M30, andcf3M30, andcf3M30, andcf3M30, andcf3M30,
    andcf3M30, orcf3M30, orcf3M30, orcf3M30, orcf3M30, orcf3M30, orcf3M30,
    orcf3M30, orcf3M30, xorcf3M30, xorcf3M30, xorcf3M30, xorcf3M30, xorcf3M30,
    xorcf3M30, xorcf3M30, xorcf3M30, ldcf3M30, ldcf3M30, ldcf3M30, ldcf3M30,
    ldcf3M30, ldcf3M30, ldcf3M30, ldcf3M30, stcf3M30, stcf3M30, stcf3M30,
    stcf3M30, stcf3M30, stcf3M30, stcf3M30, stcf3M30, tset3M30, tset3M30,
    tset3M30, tset3M30, tset3M30, tset3M30, tset3M30, tset3M30, res3M30,
    res3M30, res3M30, res3M30, res3M30, res3M30, res3M30, res3M30, set3M30,
    set3M30, set3M30, set3M30, set3M30, set3M30, set3M30, set3M30,
    //
    ...
    jpccM300, jpccM301, jpccM302, jpccM303, jpccM304, jpccM305, jpccM306,
    jpccM307, jpccM308, jpccM309, jpccM30A, jpccM30B, jpccM30C, jpccM30D,
    jpccM30E, jpccM30F, callccM300, callccM301, callccM302, callccM303,
    callccM304, callccM305, callccM306, callccM307, callccM308, callccM309,
    callccM30A, callccM30B, callccM30C, callccM30D, callccM30E, callccM30F,
    retcc0, retcc1, retcc2, retcc3, retcc4, retcc5, retcc6, retcc7, retcc8,
    retcc9, retccA, retccB, retccC, retccD, retccE, retccF};
// x0111xxx xxxxxxxx
...
DECODE_TABLE int (*decode_tableC8[256])() = {
    udef, udef, udef, ldrIB, pushrB, poprB, cplrB, negrB, mulrIB, mulsrIB,
    divrIB, divsrIB, udef, udef, udef, udef, daar, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, bios, udef, djnzB, udef, udef, udef, andcf4rB,
    orcf4rB, xorcf4rB, ldcf4rB, stcf4rB, udef, udef, udef, andcfArB, orcfArB,
    xorcfArB, ldcfArB, stcfArB, udef, ldccrB, ldcrcB, res4rB, set4rB, chg4rB,
    bit4rB, tset4rB, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef,
    //
    mulRrB, mulRrB, mulRrB, mulRrB, mulRrB, mulRrB, mulRrB, mulRrB, mulsRrB,
    mulsRrB, mulsRrB, mulsRrB, mulsRrB, mulsRrB, mulsRrB, mulsRrB, divRrB,
    divRrB, divRrB, divRrB, divRrB, divRrB, divRrB, divRrB, divsRrB, divsRrB,
    divsRrB, divsRrB, divsRrB, divsRrB, divsRrB, divsRrB, inc3rB, inc3rB,
    inc3rB, inc3rB, inc3rB, inc3rB, inc3rB, inc3rB, dec3rB, dec3rB, dec3rB,
    dec3rB, dec3rB, dec3rB, dec3rB, dec3rB, sccB0, sccB1, sccB2, sccB3, sccB4,
    sccB5, sccB6, sccB7, sccB8, sccB9, sccBA, sccBB, sccBC, sccBD, sccBE, sccBF,
    //
    addRrB, addRrB, addRrB, addRrB, addRrB, addRrB, addRrB, addRrB, ldRrB,
    ldRrB, ldRrB, ldRrB, ldRrB, ldRrB, ldRrB, ldRrB, adcRrB, adcRrB, adcRrB,
    adcRrB, adcRrB, adcRrB, adcRrB, adcRrB, ldrRB, ldrRB, ldrRB, ldrRB, ldrRB,
    ldrRB, ldrRB, ldrRB, subRrB, subRrB, subRrB, subRrB, subRrB, subRrB, subRrB,
    subRrB, ldr3B, ldr3B, ldr3B, ldr3B, ldr3B, ldr3B, ldr3B, ldr3B, sbcRrB,
    sbcRrB, sbcRrB, sbcRrB, sbcRrB, sbcRrB, sbcRrB, sbcRrB, exRrB, exRrB, exRrB,
    exRrB, exRrB, exRrB, exRrB, exRrB,
    //
    andRrB, andRrB, andRrB, andRrB, andRrB, andRrB, andRrB, andRrB, addrIB,
    adcrIB, subrIB, sbcrIB, andrIB, xorrIB, orrIB, cprIB, xorRrB, xorRrB,
    xorRrB, xorRrB, xorRrB, xorRrB, xorRrB, xorRrB, cpr3B, cpr3B, cpr3B, cpr3B,
    cpr3B, cpr3B, cpr3B, cpr3B, orRrB, orRrB, orRrB, orRrB, orRrB, orRrB, orRrB,
    orRrB, rlc4rB, rrc4rB, rl4rB, rr4rB, sla4rB, sra4rB, sll4rB, srl4rB, cpRrB,
    cpRrB, cpRrB, cpRrB, cpRrB, cpRrB, cpRrB, cpRrB, rlcArB, rrcArB, rlArB,
    rrArB, slaArB, sraArB, sllArB, srlArB};
// x1010xxx xxxxxxxx
...
DECODE_TABLE int (*decode_tableD8[256])() = {
    udef, udef, udef, ldrIW, pushrW, poprW, cplrW, negrW, mulrIW, mulsrIW,
    divrIW, divsrIW, udef, udef, bs1f, bs1b, udef, udef, extzrW, extsrW, paarW,
    udef, mirrr, udef, udef, mular, udef, udef, djnzW, udef, udef, udef,
    andcf4rW, orcf4rW, xorcf4rW, ldcf4rW, stcf4rW, udef, udef, udef, andcfArW,
    orcfArW, xorcfArW, ldcfArW, stcfArW, udef, ldccrW, ldcrcW, res4rW, set4rW,
    chg4rW, bit4rW, tset4rW, udef, udef, udef, minc1, minc2, minc4, udef, mdec1,
    mdec2, mdec4, udef,
    //
    mulRrW, mulRrW, mulRrW, mulRrW, mulRrW, mulRrW, mulRrW, mulRrW, mulsRrW,
    mulsRrW, mulsRrW, mulsRrW, mulsRrW, mulsRrW, mulsRrW, mulsRrW, divRrW,
    divRrW, divRrW, divRrW, divRrW, divRrW, divRrW, divRrW, divsRrW, divsRrW,
    divsRrW, divsRrW, divsRrW, divsRrW, divsRrW, divsRrW, inc3rW, inc3rW,
    inc3rW, inc3rW, inc3rW, inc3rW, inc3rW, inc3rW, dec3rW, dec3rW, dec3rW,
    dec3rW, dec3rW, dec3rW, dec3rW, dec3rW, sccW0, sccW1, sccW2, sccW3, sccW4,
    sccW5, sccW6, sccW7, sccW8, sccW9, sccWA, sccWB, sccWC, sccWD, sccWE, sccWF,
    //
    addRrW, addRrW, addRrW, addRrW, addRrW, addRrW, addRrW, addRrW, ldRrW,
    ldRrW, ldRrW, ldRrW, ldRrW, ldRrW, ldRrW, ldRrW, adcRrW, adcRrW, adcRrW,
    adcRrW, adcRrW, adcRrW, adcRrW, adcRrW, ldrRW, ldrRW, ldrRW, ldrRW, ldrRW,
    ldrRW, ldrRW, ldrRW, subRrW, subRrW, subRrW, subRrW, subRrW, subRrW, subRrW,
    subRrW, ldr3W, ldr3W, ldr3W, ldr3W, ldr3W, ldr3W, ldr3W, ldr3W, sbcRrW,
    sbcRrW, sbcRrW, sbcRrW, sbcRrW, sbcRrW, sbcRrW, sbcRrW, exRrW, exRrW, exRrW,
    exRrW, exRrW, exRrW, exRrW, exRrW,
    //
    andRrW, andRrW, andRrW, andRrW, andRrW, andRrW, andRrW, andRrW, addrIW,
    adcrIW, subrIW, sbcrIW, andrIW, xorrIW, orrIW, cprIW, xorRrW, xorRrW,
    xorRrW, xorRrW, xorRrW, xorRrW, xorRrW, xorRrW, cpr3W, cpr3W, cpr3W, cpr3W,
    cpr3W, cpr3W, cpr3W, cpr3W, orRrW, orRrW, orRrW, orRrW, orRrW, orRrW, orRrW,
    orRrW, rlc4rW, rrc4rW, rl4rW, rr4rW, sla4rW, sra4rW, sll4rW, srl4rW, cpRrW,
    cpRrW, cpRrW, cpRrW, cpRrW, cpRrW, cpRrW, cpRrW, rlcArW, rrcArW, rlArW,
    rrArW, slaArW, sraArW, sllArW, srlArW};
// x1100xxx xxxxxxxx
...
DECODE_TABLE int (*decode_tableE8[256])() = {
    udef, udef, udef, ldrIL, pushrL, poprL, udef, udef, udef, udef, udef, udef,
    link, unlk, udef, udef, udef, udef, extzrL, extsrL, paarL, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, ldccrL, ldcrcL,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef,
    //
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, inc3rL, inc3rL, inc3rL,
    inc3rL, inc3rL, inc3rL, inc3rL, inc3rL, dec3rL, dec3rL, dec3rL, dec3rL,
    dec3rL, dec3rL, dec3rL, dec3rL, udef, udef, udef, udef, udef, udef, udef,
    udef, udef, udef, udef, udef, udef, udef, udef, udef,
    //
    addRrL, addRrL, addRrL, addRrL, addRrL, addRrL, addRrL, addRrL, ldRrL,
    ldRrL, ldRrL, ldRrL, ldRrL, ldRrL, ldRrL, ldRrL, adcRrL, adcRrL, adcRrL,
    adcRrL, adcRrL, adcRrL, adcRrL, adcRrL, ldrRL, ldrRL, ldrRL, ldrRL, ldrRL,
    ldrRL, ldrRL, ldrRL, subRrL, subRrL, subRrL, subRrL, subRrL, subRrL, subRrL,
    subRrL, ldr3L, ldr3L, ldr3L, ldr3L, ldr3L, ldr3L, ldr3L, ldr3L, sbcRrL,
    sbcRrL, sbcRrL, sbcRrL, sbcRrL, sbcRrL, sbcRrL, sbcRrL, udef, udef, udef,
    udef, udef, udef, udef, udef,
    //
    andRrL, andRrL, andRrL, andRrL, andRrL, andRrL, andRrL, andRrL, addrIL,
    adcrIL, subrIL, sbcrIL, andrIL, xorrIL, orrIL, cprIL, xorRrL, xorRrL,
    xorRrL, xorRrL, xorRrL, xorRrL, xorRrL, xorRrL, udef, udef, udef, udef,
    udef, udef, udef, udef, orRrL, orRrL, orRrL, orRrL, orRrL, orRrL, orRrL,
    orRrL, rlc4rL, rrc4rL, rl4rL, rr4rL, sla4rL, sra4rL, sll4rL, srl4rL, cpRrL,
    cpRrL, cpRrL, cpRrL, cpRrL, cpRrL, cpRrL, cpRrL, rlcArL, rrcArL, rlArL,
    rrArL, slaArL, sraArL, sllArL, srlArL};
// x1110xxx xxxxxxxx
...
int decode88(void) // (XWA+d) (XBC+d) (XDE+d) (XHL+d) (XIX+d) (XIY+d) (XIZ+d)
                   // (XSP+d) scr.B
{
...
int decode98(void) // (XWA+d) (XBC+d) (XDE+d) (XHL+d) (XIX+d) (XIY+d) (XIZ+d)
                   // (XSP+d) scr.W
{
  mem = (*cregsL[opcode & 7]) + (signed char)readbyteSetLastbyte();
  memW = mem_readW(mem);
...
int decodeA8(void) // (XWA+d) (XBC+d) (XDE+d) (XHL+d) (XIX+d) (XIY+d) (XIZ+d)
                   // (XSP+d) scr.L
{
  mem = (*cregsL[opcode & 7]) + (signed char)readbyteSetLastbyte();
  memL = mem_readL(mem);
...
int decodeB8(
    void) // (XWA+d) (XBC+d) (XDE+d) (XHL+d) (XIX+d) (XIY+d) (XIZ+d) (XSP+d) dst
{
  mem = (*cregsL[opcode & 7]) + (signed char)readbyteSetLastbyte();
  // lastbyte = readbyte();
...
int (*instr_table[256])() = {
    nop, normal, pushsr, popsr, tmax, halt, ei, reti, ld8I, pushI, ldw8I,
    pushwI, incf, decf, ret, retd, rcf, scf, ccf, zcf, pushA, popA, exFF, ldf,
    pushF, popF, jp16, jp24, call16, call24, calr, udef, ldRIB, ldRIB, ldRIB,
    ldRIB, ldRIB, ldRIB, ldRIB, ldRIB, pushRW, pushRW, pushRW, pushRW, pushRW,
    pushRW, pushRW, pushRW, ldRIW, ldRIW, ldRIW, ldRIW, ldRIW, ldRIW, ldRIW,
    ldRIW, pushRL, pushRL, pushRL, pushRL, pushRL, pushRL, pushRL, pushRL,
    //
    ldRIL, ldRIL, ldRIL, ldRIL, ldRIL, ldRIL, ldRIL, ldRIL, popRW, popRW, popRW,
    popRW, popRW, popRW, popRW, popRW, udef, udef, udef, udef, udef, udef, udef,
    udef, popRL, popRL, popRL, popRL, popRL, popRL, popRL, popRL, jrcc0, jrcc1,
    jrcc2, jrcc3, jrcc4, jrcc5, jrcc6, jrcc7, jrcc8, jrcc9, jrccA, jrccB, jrccC,
    jrccD, jrccE, jrccF, jrlcc0, jrlcc1, jrlcc2, jrlcc3, jrlcc4, jrlcc5, jrlcc6,
    jrlcc7, jrlcc8, jrlcc9, jrlccA, jrlccB, jrlccC, jrlccD, jrlccE, jrlccF,
    //
    decode80, decode80, decode80, decode80, decode80, decode80, decode80,
    decode80, decode88, decode88, decode88, decode88, decode88, decode88,
    decode88, decode88, decode90, decode90, decode90, decode90, decode90,
    decode90, decode90, decode90, decode98, decode98, decode98, decode98,
    decode98, decode98, decode98, decode98, decodeA0, decodeA0, decodeA0,
    decodeA0, decodeA0, decodeA0, decodeA0, decodeA0, decodeA8, decodeA8,
    decodeA8, decodeA8, decodeA8, decodeA8, decodeA8, decodeA8, decodeB0,
    decodeB0, decodeB0, decodeB0, decodeB0, decodeB0, decodeB0, decodeB0,
    decodeB8, decodeB8, decodeB8, decodeBB, decodeB8, decodeB8, decodeB8,
    decodeB8,
    //
    decodeC0, decodeC1, decodeC2, decodeC3, decodeC4, decodeC5, udef, decodeC7,
    decodeC8, decodeC8, decodeC8, decodeC8, decodeC8, decodeC8, decodeC8,
    decodeC8, decodeD0, decodeD1, decodeD2, decodeD3, decodeD4, decodeD5, udef,
    decodeD7, decodeD8, decodeD8, decodeD8, decodeD8, decodeD8, decodeD8,
    decodeD8, decodeD8, decodeE0, decodeE1, decodeE2, decodeE3, decodeE4,
    decodeE5, udef, decodeE7, decodeE8, decodeE8, decodeE8, decodeE8, decodeE8,
    decodeE8, decodeE8, decodeE8, decodeF0, decodeF1, decodeF2, decodeF3,
    decodeF4, decodeF5, udef, ldx, swi, swi, swi, swi, swi, swi, swi, swi};
...
int tlcs_alloc_tables(void) {
  ...
  if (!allregsB || !allregsW || !allregsL || !cregsB || !cregsW || !cregsL) {
    free(allregsB);
    free(allregsW);
    free(allregsL);
    free(cregsB);
...
void tlcs_init(void) {
  ...
  // these are the settings for a real 900H, we will set them to what we want
  // <grin>
  ...
  allregsL[0x00] = allregsL[0x01] = allregsL[0x02] = allregsL[0x03] =
      &gen_regsXWA0;
  allregsL[0x04] = allregsL[0x05] = allregsL[0x06] = allregsL[0x07] =
      &gen_regsXBC0;
  allregsL[0x08] = allregsL[0x09] = allregsL[0x0a] = allregsL[0x0b] =
      &gen_regsXDE0;
  allregsL[0x0c] = allregsL[0x0d] = allregsL[0x0e] = allregsL[0x0f] =
      &gen_regsXHL0;
  allregsL[0x10] = allregsL[0x11] = allregsL[0x12] = allregsL[0x13] =
      &gen_regsXWA1;
  allregsL[0x14] = allregsL[0x15] = allregsL[0x16] = allregsL[0x17] =
      &gen_regsXBC1;
  allregsL[0x18] = allregsL[0x19] = allregsL[0x1a] = allregsL[0x1b] =
      &gen_regsXDE1;
  allregsL[0x1c] = allregsL[0x1d] = allregsL[0x1e] = allregsL[0x1f] =
      &gen_regsXHL1;
  allregsL[0x20] = allregsL[0x21] = allregsL[0x22] = allregsL[0x23] =
      &gen_regsXWA2;
  allregsL[0x24] = allregsL[0x25] = allregsL[0x26] = allregsL[0x27] =
      &gen_regsXBC2;
  allregsL[0x28] = allregsL[0x29] = allregsL[0x2a] = allregsL[0x2b] =
      &gen_regsXDE2;
  allregsL[0x2c] = allregsL[0x2d] = allregsL[0x2e] = allregsL[0x2f] =
      &gen_regsXHL2;
  allregsL[0x30] = allregsL[0x31] = allregsL[0x32] = allregsL[0x33] =
      &gen_regsXWA3;
  allregsL[0x34] = allregsL[0x35] = allregsL[0x36] = allregsL[0x37] =
      &gen_regsXBC3;
  allregsL[0x38] = allregsL[0x39] = allregsL[0x3a] = allregsL[0x3b] =
      &gen_regsXDE3;
  allregsL[0x3c] = allregsL[0x3d] = allregsL[0x3e] = allregsL[0x3f] =
      &gen_regsXHL3;
  allregsL[0xf0] = allregsL[0xf1] = allregsL[0xf2] = allregsL[0xf3] =
      &gen_regsXIX;
  allregsL[0xf4] = allregsL[0xf5] = allregsL[0xf6] = allregsL[0xf7] =
      &gen_regsXIY;
  allregsL[0xf8] = allregsL[0xf9] = allregsL[0xfa] = allregsL[0xfb] =
      &gen_regsXIZ;
  allregsL[0xfc] = allregsL[0xfd] = allregsL[0xfe] = allregsL[0xff] =
      &gen_regsXSP;
  ...
  //    if(realBIOSloaded)
  //        gen_regsPC = 0xFF1800;  //this is where Koyote starts when loading
  //        BIOS, but it doesn't work for me
...
static INLINE void tlcs_interrupt(int irq) {
  ...
  const static unsigned char vector[0x16] = {
      0x28, 0x70, 0x2C, 0x30, 0x34, 0x38, 0x40, 0x44, 0x48, 0x4C, 0x50,
      0x54, 0x58, 0x5C, 0x60, 0x64, 0x68, 0x6C, 0x74, 0x78, 0x7C, 0x80};
  const static unsigned char DMAvector[0x16] = {
      0x0A, 0x1C, 0x0B, 0x0C, 0x0D, 0x0E, 0x10, 0x11, 0x12, 0x13, 0x14,
      0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x00, 0x00, 0x00, 0x00};
  int i;
  int level = ((irq & 1) ? ((cpuram[0x70 + (irq >> 1)]) >> 4) & 7
                         : (cpuram[0x70 + (irq >> 1)]) & 7);
  if (level == 7)
    level = 0;
...
void tlcs_interrupt_wrapper(int irq) { tlcs_interrupt(irq); }
...
// handle DMA
static INLINE void tlcsDMAchannel(unsigned char *mode, unsigned int *src,
                                  unsigned int *dest, unsigned short *count,
                                  unsigned char *vector, int channel) {
  switch (*mode) {
  case 0x00:
...
// #define TIMER_BASE_RATE  (56*2)  //this seems to be okay for PacMan, but CFC2
// is horribly sped up
...
// 16bit combined timer mode not supported yet (might be easy to implement
// though)
static INLINE void tlcs1Timer(int stateChange, int timerNr, int run, int mode,
                              int *t0, int *t1, int check0, int check1) {
...
static INLINE void tlcsTimers(int stateChange) {
  ...
  // timer 2 + 3
  tlcs1Timer(stateChange, 2, (T8RUN >> 2) & 3, T23MOD, &timer2, &timer3, TREG2,
             TREG3);
}
...
static void tlcsTI0(void) {
  ...
  if (gfx_hacks == 1) {
    ...
    if ((mainrom[0x000020] == 0x11 || mainrom[0x000020] == 0x10) &&
        *scanlineY > 182)
      timer0 = 0;
...
static int tlcs_step(void) {
  ...
  if (interruptPendingLevel > (unsigned char)((gen_regsSR & 0x7000) >> 12)) {
    ...
    gen_regsPC =
        mem_readL(0x00FFFF00 + pendingInterrupts[interruptPendingLevel - 1][0]);
    my_pc = get_address(gen_regsPC);
  ...
  while (cycles > 0) {
    ...
    if (hCounter < 0) {
...
#ifdef NGP_ONLY_RENDER_VISIBLE_LINES
        if (s_lut_y_render[*scanlineY]) // only use pattern if this is a visible
                                        // line
#endif
          renderThisLine = s_framePatternIdx;
#ifdef NGP_ONLY_RENDER_VISIBLE_LINES
      ...
      if (*scanlineY < 152)
        if (!s_lut_y_render[*scanlineY])
          renderThisLine =
              false; // do not render lines that arent visible due to scaling
#endif
#endif
#endif
      // per-line rendering debug. actual scanlineY values will go up to ~198
      // if (*scanlineY < 152)
      //     printf("[NGP_LINE] scanelineY %d | renderThisLine %d |
      //     s_framePatternIdx %d |
...
#ifdef NGP_HW_INTERLACED
          s_interlace_parity =
              s_framePatternIdx; // <-- set parity for draw, if 152 lines than
                                 // the parity the same as the last line
#endif
...
#ifdef NGP_ONLY_RENDER_VISIBLE_LINES
        s_interlace_parity =
            s_framePatternIdx ^ 1; // the opposite parity of the current value
                                   // for odd number of line(135)
#else
        s_interlace_parity =
            s_framePatternIdx; // the current pairty for the draw parity for
                               // even number of lines(152)
#endif
#endif
#endif
      }
