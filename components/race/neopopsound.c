...
_u16 *dacBufferL = NULL;

// Compteurs / index
int dacLBufferRead = 0;
int dacLBufferWrite = 0;
int dacLBufferCount = 0;
int fixsoundmahjong;
...
/* noise feedback for white noise mode (verified on real SN76489 by John
 * Kortink) */
...
static _u16 sample_chip_tone(void) {
  ...
  for (i = 0; i < 3; i++) {
    if (toneChip.Output[i])
      vol[i] += toneChip.Count[i];
  ...
  if (data & 0x80) {
      ...
      if (filter_sound) {
        if (chip == &toneChip)
          system_debug_message(
              "sound (T): Set Tone %d Volume to %d (0 = min, 15 = max)", c,
              15 - (data & 0xF));
        else
          system_debug_message(
              "sound (N): Set Tone %d Volume to %d (0 = min, 15 = max)", c,
              15 - (data & 0xF));
      }
#endif
    ...
    case 6: /* noise  : frequency, mode */
    {
      ...
      if (filter_sound) {
        ...
        if (chip == &toneChip)
          system_debug_message("sound (T): Set Noise Mode to %s, Period = %s",
                               nm, pm);
        else
          system_debug_message("sound (N): Set Noise Mode to %s, Period = %s",
                               nm, pm);
      }
#endif
      ...
      chip->Period[3] =
          (n == 3) ? 2 * chip->Period[2] : (UpdateStep << (5 + n));
    ...
    case 4: /* tone 2 : frequency */
      ...
      if (filter_sound) {
        if (chip == &toneChip)
          system_debug_message("sound (T): Set Tone %d Frequency to %d", c,
                               chip->Register[r]);
        else
          system_debug_message("sound (N): Set Tone %d Frequency to %d", c,
                               chip->Register[r]);
      }
#endif
      break;
