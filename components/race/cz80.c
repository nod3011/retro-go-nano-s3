...
static u8
    *SZXY_BIT; /* zero, sign and parity/overflow (=zero) flags for BIT opcode */
...
void Cz80_Init(cz80_struc *cpu) {
  ...
  for (i = 0; i < 256; i++) {
    ...
    SZXYHV_inc[i] = SZXY[i];
    if (i == 0x80)
      SZXYHV_inc[i] |= CZ80_VF;
    if ((i & 0x0F) == 0x00)
      SZXYHV_inc[i] |= CZ80_HF;

    SZXYHV_dec[i] = SZXY[i] | CZ80_NF;
    if (i == 0x7F)
      SZXYHV_dec[i] |= CZ80_VF;
    if ((i & 0x0F) == 0x0F)
      SZXYHV_dec[i] |= CZ80_HF;
  }
}
