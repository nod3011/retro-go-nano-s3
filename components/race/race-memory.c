...
const unsigned char ngpcpuram[256] = {
    ...
    0xFF, 0x08, 0xFF, 0xFF,
    // 0x10
    0x34, 0x3C, 0xFF, 0xFF, 0xFF, 0x3F, 0x00, 0x00, 0x3F, 0xFF, 0x2D, 0x01,
    0xFF, 0xFF, 0x03, 0xB2,
    // 0x20
    0x80, 0x00, 0x01, 0x90, 0x03, 0xB0, 0x90, 0x62, 0x05, 0x00, 0x00, 0x00,
    0x0C, 0x0C, 0x4C, 0x4C,
    // 0x30
    ...
    0x00, 0x00, 0x00, 0x00,
    // 0x50
    0x00, 0x20, 0x69, 0x15, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0xFF, 0xFF, 0xFF, 0xFF,
    // 0x60
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x17, 0x17, 0x03, 0x03,
    0x02, 0x00, 0x00, 0x00,
...
bool ngp_mem_alloc_init(size_t mainram_sz, size_t cpurom_sz) {
  ...
  if (!s_cpuram_256) {
    s_cpuram_256 = (unsigned char *)alloc_aligned(0x1000);
    if (!s_cpuram_256)
      return false;
    memset(s_cpuram_256, 0, 0x1000);
  }
...
void ngp_mem_init(void) {
  ...
  if (!mainram || !cpurom) {
    if (!ngp_mem_alloc_init(0x14000, 0x10000)) {
      for (;;) {
      }
    }
    cz80_bind_memory();
  ...
  case NGPC:
    ...
    if (!loadBIOS()) {
      ...
      // in theory, if we've loaded the BIOS, we shouldn't need much of this,
      ...
      for (i = 0; i < 0x40; i++) {
        ...
        *((unsigned int *)(&cpurom[0xfe00 + 4 * i])) =
            (unsigned int)0x00ffe000 + 0x40 * i;
      }
      ...
      *((unsigned int *)(&cpurom[0xff04])) = (unsigned int)0x00fff000;
      ...
      printf("mainrom[0..7] = %02X %02X %02X %02X %02X %02X %02X %02X\n",
             mainrom[0], mainrom[1], mainrom[2], mainrom[3], mainrom[4],
             mainrom[5], mainrom[6], mainrom[7]);

      // setup interrupt vectors in RAM
      for (i = 0; i < 18; i++) {
        *((unsigned int *)(&mainram[0x2FB8 + 4 * i])) =
            (unsigned int)0x00FFF800;
      }
