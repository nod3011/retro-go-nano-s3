...
// #define gpio_set_level(num, level) (((num) & I2C) ? rg_gpio_set_level((num) & ~I2C) : (gpio_set_level)(num, level) ==
// ESP_OK) #define gpio_get_level(num, level) (((num) & I2C) ? rg_gpio_set_level((num) & ~I2C) : (gpio_get_level)(num,
// level)) #define gpio_set_direction(num, mode) (((num) & I2C) ? rg_gpio_set_direction((num) & ~I2C) :
// (gpio_set_direction)(num, level) == ESP_OK)
...
#define RG_ASSERT(cond, msg)                              \
    while (!(cond))                                       \
    {                                                     \
        RG_PANIC("Assertion failed: `" #cond "` : " msg); \
    }
#define RG_ASSERT_ARG(cond)                    \
    while (!(cond))                            \
    {                                          \
        RG_PANIC("Invalid function argument"); \
    }
