...
static rg_gui_event_t overscan_update_cb(rg_gui_option_t *option,
                                         rg_gui_event_t event) {
  if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT) {
    overscan = !overscan;
...
static rg_gui_event_t autocrop_update_cb(rg_gui_option_t *option,
                                         rg_gui_event_t event) {
  int val = autocrop;
  int max = 2;
  ...
  if (val == 0)
    strcpy(option->value, _("Never"));
  if (val == 1)
    strcpy(option->value, _("Auto"));
  if (val == 2)
    strcpy(option->value, _("Always"));
...
static rg_gui_event_t palette_update_cb(rg_gui_option_t *option,
  ...
  if (pal == NES_PALETTE_NOFRENDO)
    strcpy(option->value, _("Nofrendo"));
  if (pal == NES_PALETTE_COMPOSITE)
    strcpy(option->value, _("Composite"));
  if (pal == NES_PALETTE_NESCLASSIC)
    strcpy(option->value, _("NES Classic"));
  if (pal == NES_PALETTE_NTSC)
    strcpy(option->value, _("NTSC"));
  if (pal == NES_PALETTE_PVM)
    strcpy(option->value, _("PVM"));
  if (pal == NES_PALETTE_SMOOTH)
    strcpy(option->value, _("Smooth"));
...
static void blit_screen(uint8 *bmp) {
  ...
  // A rolling average should be used for autocrop == 1, it causes jitter in
  // some games... int crop_h = (autocrop == 2) || (autocrop == 1 &&
  // nes->ppu->left_bg_counter > 210) ? 8 : 0;
  int crop_v = (overscan) ? nes->overscan : 0;
  int crop_h = (autocrop) ? 8 : 0;
  // crop_h = (autocrop == 2) || (autocrop == 1 && nes->ppu->left_bg_counter >
  // 210) ? 8 : 0;
...
static void nsf_draw_overlay(void) {
  ...
  const rg_gui_option_t options[] = {
      ...
      {0, _("Copyright"), (char *)header->copyright, RG_DIALOG_FLAG_NORMAL,
       NULL},
      {0, _("Playing"), (char *)song, RG_DIALOG_FLAG_NORMAL, NULL},
      RG_DIALOG_END,
  };
  snprintf(song, sizeof(song), "%d / %d", nsf_current_song,
           header->total_songs);
...
static void options_handler(rg_gui_option_t *dest) {
  *dest++ = (rg_gui_option_t){0, _("Palette"), "-", RG_DIALOG_FLAG_NORMAL,
                              &palette_update_cb};
  *dest++ = (rg_gui_option_t){0, _("Overscan"), "-", RG_DIALOG_FLAG_NORMAL,
                              &overscan_update_cb};
  *dest++ = (rg_gui_option_t){0, _("Crop sides"), "-", RG_DIALOG_FLAG_NORMAL,
                              &autocrop_update_cb};
  *dest++ = (rg_gui_option_t){0, _("Sprite limit"), "-", RG_DIALOG_FLAG_NORMAL,
                              &sprite_limit_cb};
...
void nes_main(void) {
  ...
  nes = nes_init(SYS_DETECT, app->sampleRate, true,
                 RG_BASE_PATH_BIOS "/fds_bios.bin");
  if (!nes)
  ...
  ppu_setopt(PPU_LIMIT_SPRITES,
             rg_settings_get_number(NS_APP, SETTING_SPRITELIMIT, 1));
