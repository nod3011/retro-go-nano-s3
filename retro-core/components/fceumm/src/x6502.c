...
static INLINE uint8 RdRAMFast(uint32 A) { return (_DB = RAM[A]); }

static INLINE void WrRAMFast(uint32 A, uint8 V) { RAM[A] = V; }
...
void FASTAPASS(1) X6502_IRQBegin(int w) { _IRQlow |= w; }

void FASTAPASS(1) X6502_IRQEnd(int w) { _IRQlow &= ~w; }

void TriggerNMI(void) { _IRQlow |= FCEU_IQNMI; }

void TriggerNMI2(void) { _IRQlow |= FCEU_IQNMI2; }

#ifdef FCEUDEF_DEBUGGER
/* Called from debugger. */
void FCEUI_NMI(void) { _IRQlow |= FCEU_IQNMI; }

void FCEUI_IRQ(void) { _IRQlow |= FCEU_IQTEMP; }
...
void X6502_Reset(void) { _IRQlow = FCEU_IQRESET; }
...
void X6502_Power(void) {
  _count = _tcount = _IRQlow = _PC = _A = _X = _Y = _P = _PI = _DB = _jammed =
      0;
  _S = 0xFD;
...
#else
void IRAM_ATTR X6502_Run(int32 cycles)
#endif
{
...
#ifdef FCEUDEF_DEBUGGER
void X6502_Debug(void (*CPUHook)(X6502 *), uint8 (*ReadHook)(X6502 *, uint32),
