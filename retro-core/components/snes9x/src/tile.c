...
static const uint32_t odd[4][16] = {
    ...
    {0x00000000, 0x00000010, 0x00001000, 0x00001010, 0x00100000, 0x00100010,
     0x00101000, 0x00101010, 0x10000000, 0x10000010, 0x10001000, 0x10001010,
     0x10100000, 0x10100010, 0x10101000, 0x10101010},
    {0x00000000, 0x00000040, 0x00004000, 0x00004040, 0x00400000, 0x00400040,
     0x00404000, 0x00404040, 0x40000000, 0x40000040, 0x40004000, 0x40004040,
...
static const uint32_t even[4][16] = {
    ...
    {0x00000000, 0x00000020, 0x00002000, 0x00002020, 0x00200000, 0x00200020,
     0x00202000, 0x00202020, 0x20000000, 0x20000020, 0x20002000, 0x20002020,
     0x20200000, 0x20200020, 0x20202000, 0x20202020},
    {0x00000000, 0x00000080, 0x00008000, 0x00008080, 0x00800000, 0x00800080,
     0x00808000, 0x00808080, 0x80000000, 0x80000080, 0x80008000, 0x80008080,
...
static INLINE void WRITE_4PIXELS16(int32_t Offset, uint8_t *Pixels,
  ...
  __asm__ __volatile__(
      ...
      "5:                                    \n"
      ".set reorder                          \n"
      : /* output */[In8A] "=&r"(Pixel_A), [In8B] "=&r"(Pixel_B),
        [In8C] "=&r"(Pixel_C), [In8D] "=&r"(Pixel_D), [ZA] "=&r"(Depth_A),
        [ZB] "=&r"(Depth_B), [ZC] "=&r"(Depth_C), [ZD] "=&r"(Depth_D),
...
static INLINE void WRITE_4PIXELS16_FLIPPED(int32_t Offset, uint8_t *Pixels,
                                           uint16_t *ScreenColors) {
#if defined(__MIPSEL) && defined(__GNUC__) && !defined(NO_ASM)
  uint16_t *Screen = (uint16_t *)GFX.S + Offset;
  uint8_t *Depth = GFX.DB + Offset;
...
static void WRITE_4PIXELS16x2(int32_t Offset, uint8_t *Pixels,
                              uint16_t *ScreenColors) {
  uint8_t Pixel, N;
  uint16_t *Screen = (uint16_t *)GFX.S + Offset;
  uint8_t *Depth = GFX.DB + Offset;
...
void IRAM_ATTR DrawTile16(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                          uint32_t LineCount) {
  uint8_t *bp;
  TILE_PREAMBLE_VARS();
  TILE_PREAMBLE_CODE();
...
void IRAM_ATTR DrawClippedTile16(uint32_t Tile, int32_t Offset,
...
void DrawClippedTile16HalfWidth(uint32_t Tile, int32_t Offset,
  ...
  TILE_PREAMBLE_CODE();
  TILE_CLIP_PREAMBLE_CODE();
  RENDER_CLIPPED_TILE_CODE(WRITE_4PIXELS16_HALFWIDTH,
...
static void IRAM_ATTR WRITE_4PIXELS16_ADD(int32_t Offset, uint8_t *Pixels,
                                          uint16_t *ScreenColors) {
  uint8_t Pixel, N;
  uint16_t *Screen = (uint16_t *)GFX.S + Offset;
...
static void IRAM_ATTR WRITE_4PIXELS16_ADD1_2(int32_t Offset, uint8_t *Pixels,
                                             uint16_t *ScreenColors) {
  uint8_t Pixel, N;
  uint16_t *Screen = (uint16_t *)GFX.S + Offset;
  uint8_t *Depth = GFX.ZBuffer + Offset;
...
static void WRITE_4PIXELS16_SUB(int32_t Offset, uint8_t *Pixels,
  ...
  for (N = 0; N < 4; N++) {
    if (GFX.Z1 > Depth[N] && (Pixel = Pixels[N])) {
      ...
      default:
        Screen[N] =
            (uint16_t)COLOR_SUB(ScreenColors[Pixel], Screen[GFX.Delta + N]);
        break;
      }
      Depth[N] = GFX.Z2;
    }
...
static void WRITE_4PIXELS16_SUB1_2(int32_t Offset, uint8_t *Pixels,
  ...
  for (N = 0; N < 4; N++) {
    if (GFX.Z1 > Depth[N] && (Pixel = Pixels[N])) {
      ...
      default:
        Screen[N] =
            (uint16_t)COLOR_SUB1_2(ScreenColors[Pixel], Screen[GFX.Delta + N]);
        break;
      }
      Depth[N] = GFX.Z2;
    }
...
void DrawTile16Add(uint32_t Tile, int32_t Offset, uint32_t StartLine,
  ...
  case 0:
    bp = pCache + StartLine;
    for (l = LineCount; l != 0; l--, bp += 8, Screen += GFX.PPL,
        Depth += GFX.PPL, SubDepth += GFX.PPL) {
      uint8_t N;
      for (N = 0; N < 8; N++) {
...
void DrawClippedTile16Add1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
  ...
  TILE_PREAMBLE_CODE();
  TILE_CLIP_PREAMBLE_CODE();
  RENDER_CLIPPED_TILE_CODE(WRITE_4PIXELS16_ADD1_2,
...
void DrawClippedTile16Sub1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
  ...
  TILE_PREAMBLE_CODE();
  TILE_CLIP_PREAMBLE_CODE();
  RENDER_CLIPPED_TILE_CODE(WRITE_4PIXELS16_SUB1_2,
...
static void WRITE_4PIXELS16_ADDF1_2(int32_t Offset, uint8_t *Pixels,
  ...
  for (N = 0; N < 4; N++) {
    if (GFX.Z1 > Depth[N] && (Pixel = Pixels[N])) {
      if (SubDepth[N] == 1)
        Screen[N] =
            (uint16_t)(COLOR_ADD1_2(ScreenColors[Pixel], GFX.FixedColour));
      else
        Screen[N] = ScreenColors[Pixel];
      Depth[N] = GFX.Z2;
    }
...
static void WRITE_4PIXELS16_SUBF1_2(int32_t Offset, uint8_t *Pixels,
  ...
  for (N = 0; N < 4; N++) {
    if (GFX.Z1 > Depth[N] && (Pixel = Pixels[N])) {
      if (SubDepth[N] == 1)
        Screen[N] =
            (uint16_t)COLOR_SUB1_2(ScreenColors[Pixel], GFX.FixedColour);
      else
        Screen[N] = ScreenColors[Pixel];
      Depth[N] = GFX.Z2;
    }
...
void DrawClippedTile16FixedAdd1_2(uint32_t Tile, int32_t Offset,
  ...
  TILE_PREAMBLE_CODE();
  TILE_CLIP_PREAMBLE_CODE();
  RENDER_CLIPPED_TILE_CODE(WRITE_4PIXELS16_ADDF1_2,
...
void DrawClippedTile16FixedSub1_2(uint32_t Tile, int32_t Offset,
  ...
  TILE_PREAMBLE_CODE();
  TILE_CLIP_PREAMBLE_CODE();
  RENDER_CLIPPED_TILE_CODE(WRITE_4PIXELS16_SUBF1_2,
...
#define LARGE_ADD_PIXEL(s, p)                                                  \
  (Depth[z + GFX.DepthDelta]                                                   \
       ? (Depth[z + GFX.DepthDelta] != 1 ? COLOR_ADD(p, *(s + GFX.Delta))      \
...
#define LARGE_ADD_PIXEL1_2(s, p)                                               \
  ((uint16_t)(Depth[z + GFX.DepthDelta]                                        \
...
#define LARGE_SUB_PIXEL(s, p)                                                  \
  (Depth[z + GFX.DepthDelta]                                                   \
       ? (Depth[z + GFX.DepthDelta] != 1 ? COLOR_SUB(p, *(s + GFX.Delta))      \
...
#define LARGE_SUB_PIXEL1_2(s, p)                                               \
  (Depth[z + GFX.DepthDelta]                                                   \
       ? (Depth[z + GFX.DepthDelta] != 1 ? COLOR_SUB1_2(p, *(s + GFX.Delta))   \
                                         : COLOR_SUB(p, GFX.FixedColour))      \
       : p)
