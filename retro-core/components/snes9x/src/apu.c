...
void S9xSetAPUDSP(uint8_t byte) {
  ...
  case APU_P_LOW + 0x70:
    S9xSetSoundHertz(
        reg >> 4,
        (((int16_t)byte + ((int16_t)APU.DSP[reg + 1] << 8)) & FREQUENCY_MASK) *
            8);
    break;
  ...
  case APU_P_HIGH + 0x70:
    S9xSetSoundHertz(
        reg >> 4,
        ((((int16_t)byte << 8) + (int16_t)APU.DSP[reg - 1]) & FREQUENCY_MASK) *
            8);
    break;
  case APU_ADSR1 + 0x00:
  case APU_ADSR1 + 0x10:
...
void S9xFixEnvelope(int32_t channel, uint8_t gain, uint8_t adsr1,
                    uint8_t adsr2) {
  if (adsr1 & 0x80) /* ADSR mode */
  {
    ...
    if (S9xSetSoundMode(channel, MODE_ADSR))
      S9xSetSoundADSR(channel, adsr1 & 0xf, (adsr1 >> 4) & 7, adsr2 & 0x1f,
                      (adsr2 >> 5) & 7, 8);
  } /* Gain mode */
  else if (!(gain & 0x80)) {
