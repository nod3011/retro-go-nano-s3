...
#define ON_SUB(N)                                                              \
  ((GFX.r2130 & 0x30) != 0x30 && (GFX.r2130 & 2) && (GFX.r212d & (1 << N)))
  ...
  {                                                                            \
    ...
    if (IPPU.DoubleHeightPixels &&                                             \
      ...
      for (y = GFX.StartY; y <= GFX.EndY; y++) {                               \
        ...
        memcpy(SCREEN + (y * 2 + 1) * GFX.Pitch2, SCREEN + y * 2 * GFX.Pitch2, \
               GFX.Pitch2);                                                    \
        if (DO_DEPTH) {                                                        \
          /* memmove required: Same malloc, potentially overlapping addresses  \
           * [Neb] */                                                          \
          memmove(DEPTH + (y * 2 + 1) * (GFX.PPLx2 >> 1), DEPTH + y * GFX.PPL, \
                  GFX.PPLx2 >> 1);                                             \
        }                                                                      \
      }                                                                        \
    }                                                                          \
...
void DrawTile16(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                uint32_t LineCount);
void DrawClippedTile16(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                       uint32_t Width, uint32_t StartLine, uint32_t LineCount);
...
void DrawTile16x2(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                  uint32_t LineCount);
void DrawClippedTile16x2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                         uint32_t Width, uint32_t StartLine,
                         uint32_t LineCount);
void DrawTile16x2x2(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                    uint32_t LineCount);
void DrawClippedTile16x2x2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                           uint32_t Width, uint32_t StartLine,
                           uint32_t LineCount);
void DrawLargePixel16(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                      uint32_t Pixels, uint32_t StartLine, uint32_t LineCount);
...
void DrawTile16Add(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                   uint32_t LineCount);
void DrawClippedTile16Add(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                          uint32_t Width, uint32_t StartLine,
                          uint32_t LineCount);
void DrawTile16Add1_2(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                      uint32_t LineCount);
void DrawClippedTile16Add1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                             uint32_t Width, uint32_t StartLine,
                             uint32_t LineCount);
...
void DrawTile16Sub(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                   uint32_t LineCount);
void DrawClippedTile16Sub(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                          uint32_t Width, uint32_t StartLine,
                          uint32_t LineCount);
void DrawTile16Sub1_2(uint32_t Tile, int32_t Offset, uint32_t StartLine,
                      uint32_t LineCount);
void DrawClippedTile16Sub1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                             uint32_t Width, uint32_t StartLine,
                             uint32_t LineCount);
...
void DrawClippedTile16FixedSub1_2(uint32_t Tile, int32_t Offset,
                                  uint32_t StartPixel, uint32_t Width,
                                  uint32_t StartLine, uint32_t LineCount);
void DrawLargePixel16Add(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                         uint32_t Pixels, uint32_t StartLine,
                         uint32_t LineCount);
void DrawLargePixel16Add1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                            uint32_t Pixels, uint32_t StartLine,
                            uint32_t LineCount);
void DrawLargePixel16Sub(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                         uint32_t Pixels, uint32_t StartLine,
                         uint32_t LineCount);
void DrawLargePixel16Sub1_2(uint32_t Tile, int32_t Offset, uint32_t StartPixel,
                            uint32_t Pixels, uint32_t StartLine,
                            uint32_t LineCount);
  ...
  for (uint32_t r = 0; r <= MAX_RED; r++) {
    ...
    for (uint32_t g = 0; g <= MAX_GREEN; g++) {
      ...
      for (uint32_t b = 0; b <= MAX_BLUE; b++) {
        ...
        GFX.ZERO[BUILD_PIXEL2(r, g, b) & ~ALPHA_BITS_MASK] =
...
void S9xSetupOBJ(void) {
  ...
  if (!PPU.OAMPriorityRotation || !(PPU.OAMFlip & PPU.OAMAddr & 1)) {
    ...
    do {
      ...
      if (HPos > -GFX.OBJWidths[S] && HPos <= 256) {
        ...
        for (line = 0, Y = (uint8_t)(PPU.OBJ[S].VPos & 0xff); line < Height;
             Y++, line++) {
  ...
  } else /* evil FirstSprite+Y case */
  {
    ...
    // We can't afford 30K on the stack. But maybe we have a better buffer to
    // abuse?
...
static void IRAM_ATTR DrawOBJS(bool OnMain, uint8_t D) {
  struct {
    uint16_t Pos;
    bool Value;
  } Windows[7];
  ...
  if (!clipcount) {
    ...
    for (clip = 0, i = 1; clip < clipcount; clip++) {
      ...
      if (GFX.pCurrentClip->Right[clip][4] <= GFX.pCurrentClip->Left[clip][4])
        continue;
      for (j = 0; j < i && Windows[j].Pos < GFX.pCurrentClip->Left[clip][4];
           j++)
        ;
      ...
      for (j = 0; j < i && Windows[j].Pos < GFX.pCurrentClip->Right[clip][4];
           j++)
        ;
  ...
  for (Y = GFX.StartY, Offset = Y * GFX.PPL; Y <= GFX.EndY;
    ...
    int32_t S;
    for (S = GFX.OBJLines[Y].OBJ[I].Sprite; S >= 0 && I < 32;
         S = GFX.OBJLines[Y].OBJ[++I].Sprite) {
      int32_t TileInc = 1;
      int32_t TileLine;
      int32_t TileX;
      int32_t BaseTile;
      ...
      BaseTile =
          (((GFX.OBJLines[Y].OBJ[I].Line << 1) + (PPU.OBJ[S].Name & 0xf0)) &
           0xf0) |
      ...
      if (X == -256)
        X = 256;
      for (t = tiles, O = Offset + X * GFX.PixSize;
...
static void DrawBackgroundMosaic(uint32_t BGMode, uint32_t bg, uint8_t Z1,
                                 uint8_t Z2) {
  uint32_t Lines;
  uint32_t OffsetMask;
        ...
        if (BG.TileSize != 8) {
          if (Tile & H_FLIP) {
            ...
            if (Tile & V_FLIP) {
              ...
              if (Rem16 < 8)
                (*DrawLargePixelPtr)(Tile + 17 - (Quot & 1), s, HPos & 7,
            ...
            if (Tile & V_FLIP) {
              /* Vertical flip only */
              if (Rem16 < 8)
                (*DrawLargePixelPtr)(Tile + 16 + (Quot & 1), s, HPos & 7,
                                     PixWidth, VirtAlign, Lines);
              else
                (*DrawLargePixelPtr)(Tile + (Quot & 1), s, HPos & 7, PixWidth,
                                     VirtAlign, Lines);
            } else {
              /* Normal unflipped */
              if (Rem16 > 7)
...
static void DrawBackgroundOffset(uint32_t BGMode, uint32_t bg, uint8_t Z1,
                                 uint8_t Z2) {
  uint32_t Tile;
  uint16_t *SC0;
  uint16_t *SC1;
...
static void DrawBackgroundMode5(uint32_t bg, uint8_t Z1, uint8_t Z2) {
  ...
  for (Y = IPPU.Interlace ? GFX.StartY << 1 : GFX.StartY; Y <= endy;
       Y += Lines) {
    int32_t ScreenLine;
    int32_t t1;
    int32_t t2;
    ...
    for (clip = 0; clip < clipcount; clip++) {
      ...
      if (HPos & 7) {
        ...
        if (BG.TileSize == 8) {
          ...
          if (!(Tile & (V_FLIP | H_FLIP))) /* Normal, unflipped */
            ...
            if (Tile & V_FLIP) /* H & V flip */
              (*DrawHiResClippedTilePtr)(Tile + t2 + 1 - (Quot & 1), s, Offset,
                                         Count, VirtAlign, Lines);
            else /* H flip only */
              (*DrawHiResClippedTilePtr)(Tile + t1 + 1 - (Quot & 1), s, Offset,
                                         Count, VirtAlign, Lines);
          } else /* V flip only */
      ...
      for (C = Middle; C > 0;
           s += (IPPU.HalfWidthPixels ? 4 : 8), Quot++, C--) {
        Tile = READ_2BYTES(t);
        GFX.Z1 = GFX.Z2 = depths[(Tile & 0x2000) >> 13];
        if (BG.TileSize == 8) {
          if (!(Tile & H_FLIP)) /* Normal, unflipped */
      ...
      if (Count) {
        ...
        if (BG.TileSize == 8) {
          if (!(Tile & H_FLIP)) /* Normal, unflipped */
            (*DrawHiResClippedTilePtr)(Tile + (Quot & 1), s, 0, Count,
                                       VirtAlign, Lines);
          else /* H flip */
...
static void IRAM_ATTR DrawBackground(uint32_t BGMode, uint32_t bg, uint8_t Z1,
                                     uint8_t Z2) {
  uint32_t Tile;
  uint16_t *SC0;
  uint16_t *SC1;
  ...
  if (SC1 >= (uint16_t *)(Memory.VRAM + 0x10000))
    SC1 =
        (uint16_t *)&Memory.VRAM[((uint8_t *)SC1 - &Memory.VRAM[0]) % 0x10000];
    ...
    for (clip = 0; clip < clipcount; clip++) {
      ...
      if (HPos & 7) {
        ...
        if (BG.TileSize == 8)
          ...
          if (Tile & V_FLIP) /* H & V flip */
            (*DrawClippedTilePtr)(Tile + t2 + 1 - (Quot & 1), s, Offset, Count,
                                  VirtAlign, Lines);
          else /* H flip only */
            (*DrawClippedTilePtr)(Tile + t1 + 1 - (Quot & 1), s, Offset, Count,
                                  VirtAlign, Lines);
        } else /* V flip only */
          (*DrawClippedTilePtr)(Tile + t2 + (Quot & 1), s, Offset, Count,
                                VirtAlign, Lines);
      ...
      for (C = Middle; C > 0;
        ...
        if (BG.TileSize != 8) {
          if (Tile &
              H_FLIP) /* Horizontal flip, but what about vertical flip? */
          {
      ...
      if (Count) {
        ...
        if (BG.TileSize == 8)
          ...
          if (!(Tile & (V_FLIP | H_FLIP))) /* Normal, unflipped */
            (*DrawClippedTilePtr)(Tile + t1 + (Quot & 1), s, 0, Count,
                                  VirtAlign, Lines);
          else if (Tile & H_FLIP) {
            if (Tile & V_FLIP) /* H & V flip */
            ...
            (*DrawClippedTilePtr)(Tile + t2 + (Quot & 1), s, 0, Count,
                                  VirtAlign, Lines);
        }
      }
    }
...
#define RENDER_BACKGROUND_MODE7(TYPE, FUNC)                                    \
  ...
  l = &LineMatrixData[GFX.StartY];                                             \
                                                                               \
  for (Line = GFX.StartY; Line <= GFX.EndY;                                    \
  ...
  for (Line = GFX.StartY; Line <= GFX.EndY;                                    \
    ...
    if (PPU.Mode7VFlip)                                                        \
      ...
      yy = Line;                                                               \
                                                                               \
    yy += CLIP_10_BIT_SIGNED(VOffset - CentreY);                               \
    /* Make a special case for the identity matrix, since it's a common case   \
     * and */                                                                  \
    ...
    for (clip = 0; clip < ClipCount; clip++) {                                 \
      ...
      if (simpleCase) {                                                        \
        ...
        /* The bilinear interpolator: get the colors at the four points        \
         * surrounding */                                                      \
        /* the location of one point in the _sampled_ image, and weight them   \
         * according */                                                        \
        /* to their (city block) distance.  It's very smooth, but blurry with  \
         * "close up" */                                                       \
        /* points. */                                                          \
                                                                               \
        /* 460 (slightly less than 2 source pixels per displayed pixel) is an  \
         * educated */                                                         \
        /* guess for where bilinear filtering will become a poor method for    \
         * averaging. */                                                       \
        /* (When reducing the image, the weighting used by a bilinear filter   \
         * becomes */                                                          \
        /* arbitrary, and a simple mean is a better way to represent the       \
         * source image.) */                                                   \
        ...
        if ((aa < 460 && aa > -460) && (cc < 460 && cc > -460)) {              \
          ...
          for (x = startx; x != endx;                                          \
            ...
            if (GFX.Z1 > *d && (b & GFX.Mode7Mask)) {                          \
              ...
              tempColor = ((area1 * p1) + (area2 * p2) + (area3 * p3) +        \
                           (area4 * p4)) >>                                    \
                          5;                                                   \
              theColor = (tempColor & FIRST_THIRD_COLOR_MASK) |                \
          ...
          int32_t x;                                                           \
          /* Find the dimensions of the square in the source image whose       \
           * corners will be examined. */                                      \
          uint32_t aaDelX = aa >> 1;                                           \
          uint32_t ccDelX = cc >> 1;                                           \
          uint32_t bbDelY = l->MatrixB >> 1;                                   \
          uint32_t ddDelY = l->MatrixD >> 1;                                   \
          /* Offset the location within the source image so that the four      \
           * sampled points */                                                 \
          /* center around where the single point would otherwise have been    \
           * drawn. */                                                         \
          BB -= (bbDelY >> 1);                                                 \
          DD -= (ddDelY >> 1);                                                 \
          AA -= (aaDelX >> 1);                                                 \
...
static uint32_t Q_INTERPOLATE(uint32_t A, uint32_t B, uint32_t C, uint32_t D) {
  ...
  uint32_t y = (A & TWO_LOW_BITS_MASK) + (B & TWO_LOW_BITS_MASK) +
...
static void DrawBGMode7Background16_i(uint8_t *Screen, int32_t bg) {
  RENDER_BACKGROUND_MODE7_i(uint16_t, theColor,
                            (ScreenColors[b & GFX.Mode7Mask]));
}

static void DrawBGMode7Background16Add_i(uint8_t *Screen, int32_t bg) {
  RENDER_BACKGROUND_MODE7_i(uint16_t,
                            *(d + GFX.DepthDelta)
                                ? (*(d + GFX.DepthDelta) != 1
...
void S9xUpdateScreen(void) {
  ...
  if (PPU.BGMode == 5 || PPU.BGMode == 6 || IPPU.Interlace ||
    ...
    if (IPPU.Interlace && !IPPU.DoubleHeightPixels) {
      ...
      for (y = (int32_t)GFX.StartY - 1; y >= 0; y--) {
        /* memmove converted: Same malloc, different addresses, and identical
         * addresses at line 0 [Neb] DS2 DMA notes: This code path is unused
         * [Neb] */
        memcpy(GFX.Screen + y * 2 * GFX.Pitch2, GFX.Screen + y * GFX.Pitch2,
               GFX.Pitch2);
        /* memmove converted: Same malloc, different addresses [Neb] */
        memcpy(GFX.Screen + (y * 2 + 1) * GFX.Pitch2,
               GFX.Screen + y * GFX.Pitch2, GFX.Pitch2);
      }
    }
  }
  ...
  if (!PPU.ForcedBlanking && ADD_OR_SUB_ON_ANYTHING &&
    ...
    GFX.FixedColour =
        BUILD_PIXEL(IPPU.XB[PPU.FixedColourRed], IPPU.XB[PPU.FixedColourGreen],
