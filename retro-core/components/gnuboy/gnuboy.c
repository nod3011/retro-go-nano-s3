...
int gnuboy_init(int samplerate, gb_audio_fmt_t audio_fmt,
                gb_video_fmt_t video_fmt, gb_video_cb_t *video_callback,
...
void gnuboy_set_framebuffer(void *buffer) { GB.video.buffer = buffer; }
...
void gnuboy_run(bool draw) {
  ...
  /* When using GB_PIXEL_PALETTED, the host should draw the frame in this
     callback because the palette can be modified below before gnuboy_run
     returns. */
...
int gnuboy_load_rom(const byte *data, size_t size) {
  ...
  const char *mbc_names[16] = {
      "MBC_NONE", "MBC_MBC1", "MBC_MBC2", "MBC_MBC3",  "MBC_MBC5", "MBC_MBC6",
      "MBC_MBC7", "MBC_HUC1", "MBC_HUC3", "MBC_MMM01", "INVALID",  "INVALID",
  ...
  cart.has_battery =
      (type == 3 || type == 6 || type == 9 || type == 13 || type == 15 ||
  ...
  MESSAGE_INFO("Cart loaded: name='%s', hw=%s, mbc=%s, romsize=%dK, "
               "ramsize=%dK, colorize=%d\n",
               cart.name, hw_types[hw.hwtype], mbc_names[cart.mbc],
               cart.romsize * 16, cart.ramsize * 8, cart.colorize);
  ...
  } else if (memcmp(cart.name, "RES EVIL GD", 11) == 0 ||
             memcmp(cart.name, "BIOHAZARDGDB", 12) == 0) {
...
int gnuboy_load_rom_file(const char *file) {
  ...
  // Gameboy color games can be very large so we preload a maximum of 256 banks
  // for faster boot Also 4/8MB games do not fully fit anyway, we need to leave
  // room for our bank manager's swapping.

  int preload = cart.romsize < 256 ? cart.romsize : 256;

  if (cart.romsize > 64 && (strncmp(cart.name, "RAYMAN", 6) == 0 ||
                            strncmp(cart.name, "NONAME", 6) == 0)) {
...
void gnuboy_free_rom(void) {
  // If cart.romFile isn't NULL it indicates that we haven't allocated those
  // buffers, don't free them.
...
int gnuboy_get_hwtype(void) { return hw.hwtype; }

void gnuboy_set_hwtype(gb_hwtype_t type) {
  // nothing for now
}

int gnuboy_get_palette(void) { return GB.video.colorize; }
...
bool gnuboy_sram_dirty(void) { return cart.sram_dirty != 0; }
...
int gnuboy_load_sram(const char *file) {
  ...
  for (int i = 0; i < cart.ramsize; i++) {
    if (fseek(f, i * 8192, SEEK_SET) == 0 &&
        fread(cart.rambanks[i], 8192, 1, f) == 1) {
  ...
  if (cart.has_rtc) {
    ...
    if (fseek(f, cart.ramsize * 8192, SEEK_SET) == 0 &&
        fread(&rtc_buf, 48, 1, f) == 1) {
      cart.rtc = (gb_rtc_t){
          .s = rtc_buf[0],
          .m = rtc_buf[1],
          .h = rtc_buf[2],
      ...
      MESSAGE_INFO("Loaded RTC section %03d %02d:%02d:%02d.\n", cart.rtc.d,
                   cart.rtc.h, cart.rtc.m, cart.rtc.s);
    }
  }
...
int gnuboy_save_sram(const char *file, bool quick_save) {
  ...
  for (int i = 0; i < cart.ramsize; i++) {
    if (!(cart.sram_saved & (1 << i)) || (cart.sram_dirty & (1 << i))) {
      if (fseek(f, i * 8192, SEEK_SET) == 0 &&
          fwrite(cart.rambanks[i], 8192, 1, f) == 1) {
  ...
  if (cart.has_rtc) {
    uint64_t rt = RTC_BASE + cart.rtc.s + (cart.rtc.m * 60) +
                  (cart.rtc.h * 3600) + (cart.rtc.d * 86400);
    uint32_t *rtp = (uint32_t *)&rt;
    uint32_t rtc_buf[12] = {
        cart.rtc.s,       cart.rtc.m,       cart.rtc.h,       cart.rtc.d,
        cart.rtc.flags,   cart.rtc.regs[0], cart.rtc.regs[1], cart.rtc.regs[2],
        cart.rtc.regs[3], cart.rtc.regs[4], rtp[0],           rtp[1],
    };
    if (fseek(f, cart.ramsize * 8192, SEEK_SET) == 0 &&
        fwrite(&rtc_buf, 48, 1, f) == 1) {
...
static int do_save_load(const char *file, bool save) {
  ...
  const svar_t svars[] = {
      ...
      I2("PC  ", &W(hw.cpu->pc)), I2("SP  ", &W(hw.cpu->sp)),
      I2("BC  ", &W(hw.cpu->bc)), I2("DE  ", &W(hw.cpu->de)),
      I2("HL  ", &W(hw.cpu->hl)), I2("AF  ", &W(hw.cpu->af)),

      I4("IME ", &hw.cpu->ime), I4("ima ", &hw.cpu->ima),
      I4("spd ", &hw.cpu->double_speed), I4("halt", &hw.cpu->halted),
      I4("div ", &hw.cpu->div), I4("tim ", &hw.cpu->timer),
      I4("lcdc", &hw.cycles), I4("snd ", &hw.snd->cycles),

      I4("ints", &hw.ilines), I4("pad ", &hw.pad), I4("hdma", &hw.hdma),
      I4("seri", &hw.serial),

      I4("mbcm", &hw.cart->bankmode), I4("romb", &hw.cart->rombank),
      I4("ramb", &hw.cart->rambank), I4("enab", &hw.cart->enableram),

      // We should pack that below. Size of components could vary per platform
      I4("rtcR", &hw.cart->rtc.sel), I4("rtcL", &hw.cart->rtc.latch),
      I4("rtcF", &hw.cart->rtc.flags), I4("rtcd", &hw.cart->rtc.d),
      I4("rtch", &hw.cart->rtc.h), I4("rtcm", &hw.cart->rtc.m),
      I4("rtcs", &hw.cart->rtc.s), I4("rtct", &hw.cart->rtc.ticks),
      I1("rtR8", &hw.cart->rtc.regs[0]), I1("rtR9", &hw.cart->rtc.regs[1]),
      I1("rtRA", &hw.cart->rtc.regs[2]), I1("rtRB", &hw.cart->rtc.regs[3]),
      I1("rtRC", &hw.cart->rtc.regs[4]),

      I4("S1on", &hw.snd->ch[0].on), I4("S1p ", &hw.snd->ch[0].pos),
      I4("S1c ", &hw.snd->ch[0].cnt), I4("S1ec", &hw.snd->ch[0].encnt),
      I4("S1sc", &hw.snd->ch[0].swcnt), I4("S1sf", &hw.snd->ch[0].swfreq),

      I4("S2on", &hw.snd->ch[1].on), I4("S2p ", &hw.snd->ch[1].pos),
      I4("S2c ", &hw.snd->ch[1].cnt), I4("S2ec", &hw.snd->ch[1].encnt),

      I4("S3on", &hw.snd->ch[2].on), I4("S3p ", &hw.snd->ch[2].pos),
      I4("S3c ", &hw.snd->ch[2].cnt),

      I4("S4on", &hw.snd->ch[3].on), I4("S4p ", &hw.snd->ch[3].pos),
      I4("S4c ", &hw.snd->ch[3].cnt), I4("S4ec", &hw.snd->ch[3].encnt),
...
int gnuboy_save_state(const char *file) { return do_save_load(file, true); }

int gnuboy_load_state(const char *file) { return do_save_load(file, false); }
