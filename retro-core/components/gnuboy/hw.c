...
static inline void mbc_write(unsigned a, byte b) {
  MESSAGE_DEBUG("mbc %d: cart bank %02X -[%04X:%02X]-> ", cart.mbc,
                cart.rombank, a, b);
...
byte gb_hw_read(unsigned a) {
  ...
  case 0xE000: // Memory mapped IO
    ...
    // However invalid address should already contain 0xFF (unless incorrectly
    // overwritten) So, for speed, we'll assume that this is correct and always
    // return REG(a)
    return REG(a & 0xFF);
    // }
  }
